...
use crate::{
    ...
    BackendContract, BayesError, BetaBinomialHierarchyModelIr, BetaBinomialHierarchyWorkerRequest,
    FitState, HierarchicalSbcFailure, HierarchicalSbcParameterDiagnostics, NutsSamplingSpec,
    WorkerBackend,
};

const REQUEST_FORMAT: &str = "marklab.numpyro_beta_binomial_hierarchy_sbc_worker_request";
const RESULT_FORMAT: &str = "marklab.numpyro_beta_binomial_hierarchy_sbc_worker_result";

#[derive(Clone, Debug, Serialize)]
pub struct BetaBinomialSbcCalibrationPolicy {
    pub replicates: u32,
    pub rank_bins: u32,
...
#[derive(Clone, Debug, Serialize)]
pub struct BetaBinomialSbcResourceLimits {
...
#[derive(Clone, Debug, Serialize)]
pub struct NumpyroBetaBinomialSbcWorkerRequest {
    ...
    pub calibration: BetaBinomialSbcCalibrationPolicy,
    pub resources: BetaBinomialSbcResourceLimits,
}

impl NumpyroBetaBinomialSbcWorkerRequest {
    ...
    pub fn new(
            ...
            || !(1..=3_600).contains(&timeout_seconds)
        {
            return Err(BayesError::InvalidSpec(
                "beta-binomial SBC controls or identities are invalid".into(),
            ));
        }
        ...
        let simulated_trials = u64::from(replicates)
            ...
            .ok_or_else(|| BayesError::InvalidSpec("beta-binomial SBC work overflow".into()))?;
        ...
        let total_iterations = u64::from(replicates)
            ...
            .ok_or_else(|| {
                BayesError::InvalidSpec("beta-binomial SBC iterations overflow".into())
            })?;
        if simulated_trials > maximum_simulated_trials
            || total_iterations > maximum_total_iterations
        {
            return Err(BayesError::InvalidSpec(
                "beta-binomial SBC exceeds trial or iteration limits".into(),
            ));
        }
        ...
        Ok(Self {
            ...
            source_request,
            calibration: BetaBinomialSbcCalibrationPolicy {
                replicates,
                rank_bins: 10,
            ...
            resources: BetaBinomialSbcResourceLimits {
...
pub struct BetaBinomialSbcReplicate {
    pub replicate: u32,
    pub true_population_probability: f64,
    pub true_concentration: f64,
    pub true_patient_probability_0: f64,
    pub population_probability_rank: u32,
    pub concentration_rank: u32,
    pub patient_probability_0_rank: u32,
    pub population_probability_covered: bool,
...
#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct BetaBinomialSbcDiagnostics {
    pub population_probability: HierarchicalSbcParameterDiagnostics,
...
#[serde(deny_unknown_fields)]
pub struct NumpyroBetaBinomialSbcWorkerResult {
    format: String,
    version: u32,
    ...
    pub replicates: Vec<BetaBinomialSbcReplicate>,
    pub failures: Vec<HierarchicalSbcFailure>,
    pub diagnostics: BetaBinomialSbcDiagnostics,
}

impl NumpyroBetaBinomialSbcWorkerResult {
    pub fn validate(
        &self,
        request: &NumpyroBetaBinomialSbcWorkerRequest,
        request_sha256: &str,
    ) -> Result<(), BayesError> {
                ...
                != request.calibration.replicates as usize
        {
            return Err(BayesError::WorkerContract(
                "beta-binomial SBC result identity or dimensions mismatch".into(),
            ));
        }
        ...
        for row in &self.replicates {
            if row.replicate >= request.calibration.replicates
                ...
                || [
                    row.population_probability_rank,
                ...
                || ![
                    row.true_population_probability,
                ...
                || !(0.0..1.0).contains(&row.true_population_probability)
            ...
            {
                return Err(BayesError::WorkerContract(
                    "beta-binomial SBC replicate is invalid".into(),
                ));
            }
        }
        for failure in &self.failures {
            ...
            {
                return Err(BayesError::WorkerContract(
                    "beta-binomial SBC failure disposition is invalid".into(),
                ));
            }
        }
        let diagnostics = [
            &self.diagnostics.population_probability,
        ...
        for diagnostic in diagnostics {
            ...
            {
                return Err(BayesError::WorkerContract(
                    "beta-binomial SBC diagnostics are invalid".into(),
                ));
            }
        }
        ...
        {
            return Err(BayesError::WorkerContract(
                "beta-binomial SBC disposition or fit state is invalid".into(),
            ));
        }
        Ok(())
    }
    ...
    pub fn into_result(
        ...
        BetaBinomialSbcResult {
            format: "marklab.bayesian_beta_binomial_hierarchy_sbc",
            version: 1,
...
pub struct BetaBinomialSbcResult {
    pub format: &'static str,
    pub version: u32,
    pub backend: WorkerBackend,
    pub jax_version: String,
