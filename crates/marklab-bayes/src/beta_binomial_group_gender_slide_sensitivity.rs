...
use crate::{
    BayesError, BetaBinomialGroupGenderSlideHierarchyInputIdentity,
    BetaBinomialGroupGenderSlideHierarchyPosterior,
    BetaBinomialGroupGenderSlideHierarchyWorkerResult,
    BetaBinomialGroupGenderSlidePatientPosterior, BetaBinomialGroupGenderSlidePosteriorPredictive,
    FitState, NormalMeanDiagnostics, SamplingSummary, WorkerBackend,
};

#[derive(Clone, Debug)]
pub struct BetaBinomialGroupGenderSlideSensitivityScenario {
...
#[derive(Debug)]
pub struct BetaBinomialGroupGenderSlideSensitivityScenarioRun {
    pub scenario: BetaBinomialGroupGenderSlideSensitivityScenario,
    pub result: BetaBinomialGroupGenderSlideHierarchyWorkerResult,
}

#[derive(Debug, Serialize)]
pub struct BetaBinomialGroupGenderSlideSensitivityScenarioResult {
    pub scenario_id: String,
    pub intercept_prior_sd: f64,
    pub group_effect_prior_sd: f64,
    pub gender_effect_prior_sd: f64,
    ...
    pub patient_random_effect_rms_standardized_shift: f64,
    pub maximum_absolute_standardized_shift: f64,
    pub material_shift: bool,
}

#[derive(Debug, Serialize)]
pub struct BetaBinomialGroupGenderSlideSensitivityResult {
    ...
    pub scenarios: Vec<BetaBinomialGroupGenderSlideSensitivityScenarioResult>,
    pub material_shift_scenarios: Vec<String>,
    pub seed: u64,
    pub claim_status: &'static str,
}

impl BetaBinomialGroupGenderSlideSensitivityResult {
    pub fn new(
        input: BetaBinomialGroupGenderSlideHierarchyInputIdentity,
        seed: u64,
        material_standardized_shift: f64,
        runs: Vec<BetaBinomialGroupGenderSlideSensitivityScenarioRun>,
        ...
        for run in &runs {
            if run.scenario.scenario_id.is_empty()
                ...
                || [
                    ...
                    run.scenario.patient_log_odds_sd_prior_sd,
                    run.scenario.slide_concentration_prior_sd,
                ]
                .iter()
                .any(|value| !value.is_finite() || *value <= 0.0)
            {
                return Err(BayesError::InvalidSpec(
                    "slide hierarchy sensitivity scenarios are invalid".into(),
                ));
            }
        }
        ...
        if baseline_parameters
            ...
            || baseline.patients.iter().any(|patient| {
                patient.posterior_probability.sd <= 0.0 || patient.random_log_odds_effect.sd <= 0.0
            })
        {
            return Err(BayesError::WorkerContract(
                "slide hierarchy baseline cannot scale sensitivity".into(),
            ));
        }
        let baseline_patients = baseline
            ...
            .map(|patient| {
                (
                    ...
                    patient.random_log_odds_effect.mean,
                    patient.random_log_odds_effect.sd,
                )
            })
            .collect::<Vec<_>>();
        ...
        for run in runs {
            ...
            {
                return Err(BayesError::WorkerContract(
                    "slide hierarchy sensitivity dimensions differ".into(),
                ));
            }
            ...
            for (name, (baseline_mean, baseline_sd)) in &baseline_parameters {
                let (mean, _) = current.get(name).ok_or_else(|| {
                    BayesError::WorkerContract(
                        "slide hierarchy sensitivity parameter identities differ".into(),
                    )
                })?;
                shifts.insert(name.clone(), (mean - baseline_mean) / baseline_sd);
            }
            let mut probability_squared = 0.0;
            let mut effect_squared = 0.0;
            for (patient, (id, group, gender, probability, probability_sd, effect, effect_sd)) in
                run.result.patients.iter().zip(&baseline_patients)
            {
                ...
                {
                    return Err(BayesError::WorkerContract(
                        "slide hierarchy sensitivity patient identities differ".into(),
                    ));
                }
                probability_squared +=
                    ((patient.posterior_probability.mean - probability) / probability_sd).powi(2);
                effect_squared +=
                    ((patient.random_log_odds_effect.mean - effect) / effect_sd).powi(2);
            }
            let probability_shift = (probability_squared / baseline_patients.len() as f64).sqrt();
            let effect_shift = (effect_squared / baseline_patients.len() as f64).sqrt();
            let maximum = shifts
                .values()
                .map(|value| value.abs())
                .fold(probability_shift.max(effect_shift), f64::max);
            ...
            scenarios.push(BetaBinomialGroupGenderSlideSensitivityScenarioResult {
                ...
                patient_log_odds_sd_prior_sd: run.scenario.patient_log_odds_sd_prior_sd,
                slide_concentration_prior_sd: run.scenario.slide_concentration_prior_sd,
                ...
                patient_probability_rms_standardized_shift: probability_shift,
                patient_random_effect_rms_standardized_shift: effect_shift,
        ...
        Ok(Self {
            format: "marklab.bayesian_beta_binomial_group_gender_slide_hierarchy_sensitivity",
            version: 1,
            input,
...
fn parameter_summaries(
    ...
    [
        ...
        ("patient_log_odds_sd", &posterior.patient_log_odds_sd),
        ("slide_concentration", &posterior.slide_concentration),
        (
