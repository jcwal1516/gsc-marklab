...
use crate::{
    ...
    BackendContract, BayesError, BetaBinomialGroupRegressionModelIr,
    BetaBinomialGroupRegressionWorkerRequest, FitState, HierarchicalSbcFailure,
    HierarchicalSbcParameterDiagnostics, NutsSamplingSpec, WorkerBackend,
};

const REQUEST_FORMAT: &str = "marklab.numpyro_beta_binomial_group_regression_sbc_worker_request";
const RESULT_FORMAT: &str = "marklab.numpyro_beta_binomial_group_regression_sbc_worker_result";

#[derive(Clone, Debug, Serialize)]
pub struct BetaBinomialGroupSbcCalibrationPolicy {
    pub replicates: u32,
    pub rank_bins: u32,
...
#[derive(Clone, Debug, Serialize)]
pub struct BetaBinomialGroupSbcResourceLimits {
...
#[derive(Clone, Debug, Serialize)]
pub struct NumpyroBetaBinomialGroupSbcWorkerRequest {
    ...
    pub calibration: BetaBinomialGroupSbcCalibrationPolicy,
    pub resources: BetaBinomialGroupSbcResourceLimits,
}

impl NumpyroBetaBinomialGroupSbcWorkerRequest {
    ...
    pub fn new(
            ...
            || !(1..=3_600).contains(&timeout_seconds)
        {
            return Err(BayesError::InvalidSpec(
                "beta-binomial group SBC controls or identities are invalid".into(),
            ));
        }
        ...
        let simulated_trials = u64::from(replicates)
            ...
            .ok_or_else(|| {
                BayesError::InvalidSpec("beta-binomial group SBC work overflow".into())
            })?;
        ...
        let total_iterations = u64::from(replicates)
            ...
            .ok_or_else(|| {
                BayesError::InvalidSpec("beta-binomial group SBC iterations overflow".into())
            })?;
        if simulated_trials > maximum_simulated_trials
            || total_iterations > maximum_total_iterations
        {
            return Err(BayesError::InvalidSpec(
                "beta-binomial group SBC exceeds trial or iteration limits".into(),
            ));
        }
        ...
        Ok(Self {
            ...
            source_request,
            calibration: BetaBinomialGroupSbcCalibrationPolicy {
                replicates,
                rank_bins: 10,
            ...
            resources: BetaBinomialGroupSbcResourceLimits {
...
pub struct BetaBinomialGroupSbcReplicate {
    ...
    pub probability_difference_rank: u32,
    ...
    pub probability_difference_covered: bool,
...
#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct BetaBinomialGroupSbcDiagnostics {
    ...
    pub probability_difference: HierarchicalSbcParameterDiagnostics,
    pub patient_probability_0: HierarchicalSbcParameterDiagnostics,
}

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct NumpyroBetaBinomialGroupSbcWorkerResult {
    format: String,
    version: u32,
    ...
    pub replicates: Vec<BetaBinomialGroupSbcReplicate>,
    pub failures: Vec<HierarchicalSbcFailure>,
    pub diagnostics: BetaBinomialGroupSbcDiagnostics,
}

impl NumpyroBetaBinomialGroupSbcWorkerResult {
    pub fn validate(
        &self,
        request: &NumpyroBetaBinomialGroupSbcWorkerRequest,
        request_sha256: &str,
    ) -> Result<(), BayesError> {
                ...
                != request.calibration.replicates as usize
        {
            return Err(BayesError::WorkerContract(
                "beta-binomial group SBC result identity or dimensions mismatch".into(),
            ));
        }
        ...
        for row in &self.replicates {
            if row.replicate >= request.calibration.replicates
                ...
                || [
                    ...
                    row.probability_difference_rank,
                ...
                || ![
                    ...
                    row.true_probability_difference,
                ...
                || !(-1.0..1.0).contains(&row.true_probability_difference)
            ...
            {
                return Err(BayesError::WorkerContract(
                    "beta-binomial group SBC replicate is invalid".into(),
                ));
            }
        }
        for failure in &self.failures {
            ...
            {
                return Err(BayesError::WorkerContract(
                    "beta-binomial group SBC failure disposition is invalid".into(),
                ));
            }
        }
        let diagnostics = [
            ...
            &self.diagnostics.probability_difference,
        ...
        for diagnostic in diagnostics {
            ...
            {
                return Err(BayesError::WorkerContract(
                    "beta-binomial group SBC diagnostics are invalid".into(),
                ));
            }
        }
        ...
        {
            return Err(BayesError::WorkerContract(
                "beta-binomial group SBC disposition or fit state is invalid".into(),
            ));
        }
        Ok(())
    }
    ...
    pub fn into_result(
        self,
        request: NumpyroBetaBinomialGroupSbcWorkerRequest,
    ) -> BetaBinomialGroupSbcResult {
        BetaBinomialGroupSbcResult {
            format: "marklab.bayesian_beta_binomial_group_regression_sbc",
            version: 1,
...
pub struct BetaBinomialGroupSbcResult {
    pub format: &'static str,
    pub version: u32,
    pub backend: WorkerBackend,
    pub jax_version: String,
