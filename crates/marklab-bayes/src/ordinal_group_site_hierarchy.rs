...
pub struct OrdinalGroupSitePatientData {
...
#[derive(Clone, Debug)]
pub struct OrdinalGroupSiteHierarchySpec {
    ...
    pub site_group_slope_sd_prior_sd: f64,
    pub patients: Vec<OrdinalGroupSitePatientData>,
}

#[derive(Clone, Debug, Serialize)]
pub struct OrdinalGroupSiteHierarchyModelIr {
    ...
    pub site_intercept_sd_prior_sd: f64,
    pub site_group_slope_prior: &'static str,
    pub site_group_slope_sd_prior_sd: f64,
...
#[derive(Clone, Debug, Serialize)]
pub struct OrdinalGroupSiteHierarchyResourceLimits {
...
#[derive(Clone, Debug, Serialize)]
pub struct OrdinalGroupSiteHierarchyWorkerRequest {
    pub format: &'static str,
    pub version: u32,
    pub backend: BackendContract,
    pub model: OrdinalGroupSiteHierarchyModelIr,
    pub site_ids: Vec<String>,
    pub patients: Vec<OrdinalGroupSitePatientData>,
    pub sampling: NutsSamplingSpec,
    pub resources: OrdinalGroupSiteHierarchyResourceLimits,
    pub diagnostic_policy: DiagnosticPolicy,
}

impl OrdinalGroupSiteHierarchyWorkerRequest {
    pub fn new(
        spec: OrdinalGroupSiteHierarchySpec,
        ...
        sampling.validate()?;
        let scales = [
            spec.cutpoint_prior_sd,
            spec.group_effect_prior_sd,
            spec.site_intercept_sd_prior_sd,
            spec.site_group_slope_sd_prior_sd,
        ];
            ...
            || !(1..=3_600).contains(&timeout_seconds)
        {
            return Err(BayesError::InvalidSpec(
                "ordinal site hierarchy controls or identities are invalid".into(),
            ));
        }
        ...
        {
            return Err(BayesError::InvalidSpec(
                "ordinal site hierarchy levels are invalid".into(),
            ));
        }
        ...
        for patient in &patients {
            ...
            {
                return Err(BayesError::InvalidSpec(
                    "ordinal site hierarchy patient rows are invalid".into(),
                ));
            }
            let group_index = usize::from(patient.group == spec.comparison_group);
            site_groups.entry(patient.site_id.clone()).or_default()[group_index] += 1;
        ...
        {
            return Err(BayesError::InvalidSpec(
                "ordinal site hierarchy requires 8-64 sites, two patients per group per site, and every level"
                    .into(),
            ));
        }
        ...
        {
            return Err(BayesError::InvalidSpec(
                "ordinal site hierarchy exceeds 400000 NUTS iterations".into(),
            ));
        }
        ...
        Ok(Self {
            ...
            model: OrdinalGroupSiteHierarchyModelIr {
                ...
                site_group_slope_sd_prior_sd: spec.site_group_slope_sd_prior_sd,
            ...
            patients,
            sampling,
            resources: OrdinalGroupSiteHierarchyResourceLimits {
...
pub struct OrdinalSitePosterior {
    pub site_id: String,
    pub intercept: SarScalarSummary,
    pub group_slope_deviation: SarScalarSummary,
}

#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OrdinalGroupSiteHierarchyPosterior {
    pub global_group_log_odds_effect: SarScalarSummary,
    pub site_intercept_sd: SarScalarSummary,
    pub site_group_slope_sd: SarScalarSummary,
    pub cutpoints: Vec<OrdinalCutpointPosterior>,
    pub sites: Vec<OrdinalSitePosterior>,
...
#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OrdinalGroupSiteHierarchyPosteriorPredictive {
    pub levels: Vec<OrdinalLevelPredictive>,
}

#[derive(Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OrdinalGroupSiteHierarchyWorkerResult {
    format: String,
    version: u32,
    ...
    pub posterior: OrdinalGroupSiteHierarchyPosterior,
    pub diagnostics: NormalMeanDiagnostics,
    pub posterior_predictive: OrdinalGroupSiteHierarchyPosteriorPredictive,
}

impl OrdinalGroupSiteHierarchyWorkerResult {
    pub fn validate(
        &self,
        request: &OrdinalGroupSiteHierarchyWorkerRequest,
        request_sha256: &str,
    ) -> Result<(), BayesError> {
        let levels = &request.model.ordered_levels;
        if self.format != "marklab.pymc_ordinal_group_site_hierarchy_worker_result"
            || self.version != 1
            ...
            || self.posterior_predictive.levels.len() != levels.len()
        {
            return Err(BayesError::WorkerContract(
                "ordinal site hierarchy result identity or dimensions mismatch".into(),
            ));
        }
        let expected_draws =
        ...
        {
            return Err(BayesError::WorkerContract(
                "ordinal site hierarchy sampling counts mismatch".into(),
            ));
        }
        validate_summary(&self.posterior.global_group_log_odds_effect, None)?;
        for summary in [
            &self.posterior.site_intercept_sd,
            &self.posterior.site_group_slope_sd,
        ] {
            validate_summary(summary, Some((0.0, f64::INFINITY)))?;
            if summary.mean <= 0.0 {
                return Err(BayesError::WorkerContract(
                    "ordinal site hierarchy scale is invalid".into(),
                ));
            }
        }
        validate_summary(
        ...
        for (index, cutpoint) in self.posterior.cutpoints.iter().enumerate() {
            ...
            {
                return Err(BayesError::WorkerContract(
                    "ordinal site hierarchy cutpoint order mismatch".into(),
                ));
            }
            validate_summary(&cutpoint.summary, None)?;
            previous = cutpoint.summary.mean;
        }
        for (site, site_id) in self.posterior.sites.iter().zip(&request.site_ids) {
            if site.site_id != *site_id {
                return Err(BayesError::WorkerContract(
                    "ordinal site hierarchy site order mismatch".into(),
                ));
            }
            validate_summary(&site.intercept, None)?;
            validate_summary(&site.group_slope_deviation, None)?;
        }
            ...
            .enumerate()
        {
            if posterior.level != *level || predictive.level != *level {
                return Err(BayesError::WorkerContract(
                    "ordinal site hierarchy level order mismatch".into(),
                ));
            }
            ...
            {
                return Err(BayesError::WorkerContract(
                    "ordinal site hierarchy predictive summary is invalid".into(),
                ));
            }
        }
        if (reference_sum - 1.0).abs() > 1e-6 || (comparison_sum - 1.0).abs() > 1e-6 {
            return Err(BayesError::WorkerContract(
                "ordinal site hierarchy probabilities do not sum to one".into(),
            ));
        }
        ...
        if (self.fit_state == FitState::Complete) != diagnostics_pass {
            return Err(BayesError::WorkerContract(
                "ordinal site hierarchy fit state disagrees with diagnostics".into(),
            ));
        }
        Ok(())
    }
    ...
    pub fn into_result(
        self,
        request: OrdinalGroupSiteHierarchyWorkerRequest,
        input: OrdinalGroupSiteHierarchyInputIdentity,
    ) -> OrdinalGroupSiteHierarchyResult {
        OrdinalGroupSiteHierarchyResult {
            format: "marklab.bayesian_ordinal_group_site_hierarchy",
            version: 1,
...
fn validate_summary(
    ...
    {
        return Err(BayesError::WorkerContract(
            "ordinal site hierarchy posterior summary is invalid".into(),
        ));
    }
    Ok(())
}
...
#[derive(Clone, Debug, Serialize)]
pub struct OrdinalGroupSiteHierarchyInputIdentity {
    pub path: String,
...
pub fn ordinal_group_site_data_sha256(
...
pub struct OrdinalGroupSiteHierarchyResult {
    pub format: &'static str,
    pub version: u32,
    pub backend: WorkerBackend,
    pub model: OrdinalGroupSiteHierarchyModelIr,
    pub input: OrdinalGroupSiteHierarchyInputIdentity,
    pub sampling: SamplingSummary,
    pub fit_state: FitState,
    pub posterior: OrdinalGroupSiteHierarchyPosterior,
    pub diagnostics: NormalMeanDiagnostics,
...
mod tests {
    ...
    #[test]
    fn rejects_a_site_without_both_groups() {
        let mut patients = Vec::new();
        for site in 0..8 {
            for patient in 0..4 {
                patients.push(OrdinalGroupSitePatientData {
                    patient_id: format!("s{site}-a-{patient}"),
                    site_id: format!("s{site}"),
                    group: "MSS".into(),
                    outcome_code: (patient % 2) as u32,
                });
                patients.push(OrdinalGroupSitePatientData {
                    patient_id: format!("s{site}-b-{patient}"),
                    site_id: format!("s{site}"),
                    group: if site == 7 { "MSS" } else { "MSI" }.into(),
                    outcome_code: 2 + (patient % 2) as u32,
                });
            }
        }
        assert!(matches!(
            OrdinalGroupSiteHierarchyWorkerRequest::new(
                OrdinalGroupSiteHierarchySpec {
                    ...
                    site_group_slope_sd_prior_sd: 1.0,
                    patients,
                },
                NutsSamplingSpec {
                    chains: 2,
