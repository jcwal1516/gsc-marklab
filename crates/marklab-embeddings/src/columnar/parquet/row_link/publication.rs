...
use super::{
    ...
    writer::write_cell_embedding_row_link_parquet,
};
use crate::columnar::{EmbeddingColumnarBudgets, EmbeddingColumnarPublicationError};

/// Count/hash, replay, and durably publish a fresh canonical Parquet row link.
pub fn publish_cell_embedding_row_link_parquet(
    store: &LocalArtifactStore,
    row_link: &CellEmbeddingRowLink,
    budgets: EmbeddingColumnarBudgets,
) -> Result<ArtifactPublication, EmbeddingColumnarPublicationError> {
    let summary = write_cell_embedding_row_link_parquet(&mut io::sink(), row_link, budgets)?;
    ...
    store
        .publish_new_send(&draft, |output| {
            write_cell_embedding_row_link_parquet(output, row_link, budgets)
                .map(|_| ())
                .map_err(|_| io::Error::other("canonical row-link Parquet replay failed"))
        })
