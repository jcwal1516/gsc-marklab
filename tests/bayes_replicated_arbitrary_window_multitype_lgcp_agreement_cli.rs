...
#[test]
fn pymc_and_numpyro_agree_on_the_same_replicated_multitype_intensities() {
    ...
    assert_eq!(
        result["format"],
        "marklab.replicated_arbitrary_window_multitype_lgcp_backend_agreement"
    );
    ...
    assert_eq!(
        result["comparison"]["patient_type_effects"]["element_count"],
        24
    );
    assert_eq!(
        result["comparison"]["pattern_type_effects"]["element_count"],
        48
    );
