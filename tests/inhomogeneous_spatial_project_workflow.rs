use marklab::{
    ...
    DurableProjectLimits, InhomogeneousSpatialAnalysisNode, InhomogeneousSpatialConfig,
    InhomogeneousSpatialLimits, InhomogeneousSpatialResult, LocalScheduler, MarklabProject,
...
fn runtime() -> NativeRuntimeProvenance {
    NativeRuntimeProvenance::new(
        ...
        "rustc 1.96.0-test",
        vec!["test".into()],
        ArtifactRef::from_bytes("application/vnd.marklab.executable", b"inhomogeneous-test")
            .expect("executable"),
    )
    .expect("runtime")
}

fn run_once(path: &std::path::Path, seed: u64) -> (CacheStatus, InhomogeneousSpatialResult, usize) {
    let pattern = Pattern::from_arrays(
        vec![2.0, 3.0, 7.0, 9.0],
        vec![2.0, 2.0, 7.0, 7.0],
        vec![0; 4],
        PatternMeta {
            case_id: "durable-inhomogeneous".into(),
    ...
    let node = InhomogeneousSpatialAnalysisNode::new(
        &mut project,
        NodeId::new("inhomogeneous-spatial").expect("node ID"),
        &pattern,
        &window,
        &config,
    )
    ...
    let run = execute_algorithm(
        ...
        &node,
        &scheduler,
        ArtifactSchema::new("marklab.inhomogeneous_spatial", 1).expect("schema"),
        runtime(),
    )
    .expect("run");
...
#[test]
fn inhomogeneous_spatial_reopens_as_a_verified_hit_and_seed_change_misses() {
