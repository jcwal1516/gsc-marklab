...
#[test]
fn covariate_permutation_cli_restricts_residuals_to_exact_patient_blocks() {
    let directory = tempfile::tempdir().expect("tempdir");
    let input = directory.path().join("blocked-patients.csv");
    fs::write(
        &input,
        "patient_id,group,outcome,covariate,block\n\
a-1,A,2.0,0,north\n\
a-2,A,5.5,1,north\n\
a-3,A,6.0,2,north\n\
b-1,B,-1.0,0,north\n\
