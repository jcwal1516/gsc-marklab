...
use marklab::{
    ...
    MarkWeightedKAnalysisNode, MarkWeightedKConfig, MarkWeightedKLimits, MarkWeightedKResult,
    MeasurementStatus, MissingnessPolicy, NativeRuntimeProvenance, NodeId,
    NucleusAreaUm2MarkDeclaration, ObservationWindow2D, ObservationWindowLimits, ScalarMarkColumn,
...
fn runtime() -> NativeRuntimeProvenance {
    NativeRuntimeProvenance::new(
        ...
        ArtifactRef::from_bytes(
            ...
            b"mark-weighted-k-workflow-test",
        )
        .expect("executable"),
    )
    .expect("runtime")
...
fn run_once(
    ...
    seed: u64,
) -> (CacheStatus, MarkWeightedKResult, usize) {
    ...
    let binary_provenance = publish_record(
        &mut fixture,
        b"weighted-k-project-binary-provenance",
        MARK_SCHEMA,
        1,
        None,
        Vec::new(),
    ...
    let continuous_provenance = publish_record(
        &mut fixture,
        b"weighted-k-project-area-provenance",
        MARK_SCHEMA,
        1,
        None,
        Vec::new(),
    ...
    let config = MarkWeightedKConfig::new(
        mark_id,
        vec![0.5, 1.1],
        31,
        seed,
    ...
    .expect("config");
    let node = MarkWeightedKAnalysisNode::new(
        &mut fixture.project,
        NodeId::new("mark-weighted-k").expect("node ID"),
        &input,
        &window,
        &config,
    )
    ...
    let run = execute_algorithm_with_store(
        ...
        &node,
        &scheduler,
        ArtifactSchema::new("marklab.mark_weighted_k", 1).expect("schema"),
        runtime(),
        &fixture.store,
    )
    .expect("run");
...
#[test]
fn weighted_k_reopens_as_a_verified_hit_and_seed_change_misses() {
