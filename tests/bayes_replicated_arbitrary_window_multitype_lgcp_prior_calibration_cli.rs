...
#[test]
fn replicated_multitype_lgcp_prior_generator_matches_analytic_oracles() {
    ...
    Command::cargo_bin("marklab")
        ...
        .args([
            ...
            "0.06",
            "--maximum-field-covariance-rmse-scale",
            "0.08",
    ...
    assert_eq!(
        result["format"],
        "marklab.replicated_arbitrary_window_multitype_lgcp_prior_calibration"
    );
    assert_eq!(result["fit_state"], "complete", "{result}");
    assert_eq!(result["moment_checks"].as_array().unwrap().len(), 5);
