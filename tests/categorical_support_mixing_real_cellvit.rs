...
use marklab::{
    ...
    CategoricalSupportMixingAnalysisNode, CategoricalSupportMixingConfig,
    CategoricalSupportMixingLimits, DeclaredScalarPatternInput, DurableProject,
...
const CHILD_PROJECT: &str = "MARKLAB_REAL_CELLVIT_SUPPORT_MIXING_CHILD_PROJECT";
const CHILD_RESULT: &str = "MARKLAB_REAL_CELLVIT_SUPPORT_MIXING_CHILD_RESULT";
...
fn admitted_cellvit_support_mixing_replays_across_fresh_processes() {
    ...
    for output in [&first_path, &second_path] {
        let status = Command::new(&executable)
            .arg("--exact")
            .arg("admitted_cellvit_support_mixing_child")
    ...
    assert_eq!(result["matrix"].as_array().expect("matrix").len(), 16);
    assert_eq!(
    ...
    let retained = result["total_support_retention_fraction"]
        .as_f64()
        .expect("retention fraction");
    assert!((0.0..=1.0).contains(&retained));
}

#[test]
fn admitted_cellvit_support_mixing_child() {
...
fn run_real_once(project_path: &Path, cells_path: &Path, window_path: &Path, output_path: &Path) {
    ...
    let config = CategoricalSupportMixingConfig::new(
        50.0,
        CategoricalSupportMixingLimits::new(2_000, 8, 4_000_000, 128 << 20).expect("limits"),
    )
    .expect("config");
    let categorical = ScalarMarkId::new("histologic_compartment").expect("categorical mark ID");
    let support = ScalarMarkId::new("cellvit_winning_type_pixel_support").expect("support mark ID");
    let node = CategoricalSupportMixingAnalysisNode::new(
        &mut fixture.project,
        NodeId::new("real-cellvit-categorical-support-mixing").expect("node ID"),
        &input,
        &fixture.window,
        &categorical,
        &support,
...
fn runtime() -> NativeRuntimeProvenance {
    NativeRuntimeProvenance::new(
        ...
        None,
        "rustc 1.96.0",
        vec!["real-cellvit-categorical-support".into()],
        ArtifactRef::from_bytes(
            "application/vnd.marklab.executable",
            b"categorical-support-mixing-real-cellvit-v1",
        )
        .expect("executable"),
    )
    .expect("runtime")
