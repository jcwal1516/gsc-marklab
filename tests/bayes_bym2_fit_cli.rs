...
#[test]
fn bym2_fit_uses_unit_typical_icar_scale_and_mixing_fraction() {
...
r11,194,100,1\n",
    ...
    let result: serde_json::Value =
        serde_json::from_slice(&fs::read(output).unwrap()).expect("BYM2 fit JSON");
    assert_eq!(result["format"], "marklab.bayesian_bym2_fit");
    assert_eq!(result["fit_state"], "complete");
    assert!(
        (result["icar_scaling"]["scaled_typical_marginal_variance"]
            .as_f64()
            .unwrap()
            - 1.0)
            .abs()
    ...
    assert!(result["posterior"]["sigma"]["mean"].as_f64().unwrap() > 0.0);
    assert!((0.0..=1.0).contains(&result["posterior"]["phi"]["mean"].as_f64().unwrap()));
    assert!(
