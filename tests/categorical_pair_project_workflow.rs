...
use marklab::{
    ...
    CategoricalPairResult, DeclaredScalarPatternInput, DurableProject, DurableProjectLimits,
...
fn runtime() -> NativeRuntimeProvenance {
    NativeRuntimeProvenance::new(
        ...
        ArtifactRef::from_bytes(
            ...
            b"categorical-pair-workflow-test",
        )
        .expect("executable"),
    )
    .expect("runtime")
...
fn run_once(
    ...
    seed: u64,
) -> (CacheStatus, CategoricalPairResult, usize) {
    ...
    let binary_provenance = publish_record(
        &mut fixture,
        b"categorical-project-binary-provenance",
        MARK_SCHEMA,
        1,
        None,
        Vec::new(),
    ...
    let category_provenance = publish_record(
        &mut fixture,
        b"categorical-project-category-provenance",
        MARK_SCHEMA,
        1,
        None,
        Vec::new(),
    ...
    let node = CategoricalPairAnalysisNode::new(
        &mut fixture.project,
        NodeId::new("categorical-pair").expect("node ID"),
        &input,
        &window,
        &config,
    )
...
#[test]
fn categorical_pair_reopens_as_a_verified_hit_and_seed_change_misses() {
