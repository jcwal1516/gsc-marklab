#!/usr/bin/env python3
"""Static PyMC worker for patient-unit beta-binomial group regression."""
...
def validate(request: Any, lock_digest: str, worker_digest: str) -> dict[str, Any]:
    ...
    model = obj(
        request["model"],
        {
            ...
            "intercept_prior", "intercept_prior_mean", "intercept_prior_sd",
            "group_effect_prior", "group_effect_prior_sd", "concentration_prior",
            "concentration_prior_sd", "patient_probability", "likelihood",
            "inference_parameterization", "biological_unit", "backend_capability", "maturity",
        },
        "model",
    )
    exact(model["format"], "marklab.bayesian_model_ir", "model.format")
    exact(model["version"], 1, "model.version")
    exact(model["family"], "patient_beta_binomial_group_regression", "model.family")
    ...
    exact(model["patient_probability"], "beta_group_mean_concentration", "model.patient")
    exact(model["likelihood"], "beta_binomial_collapsed_patient_counts", "model.likelihood")
    exact(
        model["inference_parameterization"],
        "collapsed_with_exact_conditional_patient_draws",
    ...
    if min(intercept_sd, group_sd, concentration_sd) <= 0.0:
...
def seed_for(seed: int, purpose: str, index: int = 0) -> int:
    digest = hashlib.sha256(
        f"marklab-pymc-beta-binomial-group-v1\0{seed}\0{purpose}\0{index}".encode()
    ).digest()
...
def fit(config: dict[str, Any], request_sha: str, lock_sha: str, worker_sha: str) -> dict[str, Any]:
    with pm.Model():
        intercept = pm.Normal(
            "intercept_log_odds", config["intercept_mean"], config["intercept_sd"]
        )
        ...
        group_probability = pm.math.sigmoid(intercept + config["indicator"] * effect)
        pm.BetaBinomial(
            "successes",
            alpha=group_probability * concentration,
            beta=(1.0 - group_probability) * concentration,
            n=config["trials"],
    ...
    effect_draws = np.asarray(posterior["posterior"]["group_log_odds_effect"].values, dtype=np.float64)
    ...
    monitored = ["intercept_log_odds", "group_log_odds_effect", "concentration"]
    prior_finite = bool(
    ...
    observed_difference = float(
        observed_proportions[comparison_mask].mean() - observed_proportions[reference_mask].mean()
    )
    ...
    return {
        "format": "marklab.pymc_beta_binomial_group_regression_worker_result",
        "version": 1,
        "backend": {
        ...
        "posterior_predictive": {
            ...
            "replicated_group_mean_proportion_difference_mean": float(replicated_differences.mean()),
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Marklab PyMC beta-binomial group worker failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
