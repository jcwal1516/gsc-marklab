#!/usr/bin/env python3
"""Prior-generative SBC for the replicated exact-window patient LGCP."""
...
REQUEST_FORMAT = "marklab.numpyro_replicated_arbitrary_window_lgcp_sbc_request"
RESULT_FORMAT = "marklab.numpyro_replicated_arbitrary_window_lgcp_sbc_result"
...
def validate(request: Any, lock_digest: str, worker_digest: str) -> dict[str, Any]:
    ...
    pymc = load_module(
        "marklab_pymc_replicated_arbitrary_window_lgcp_worker.py",
        "marklab_pymc_replicated_sbc_contract",
    )
    config = pymc.validate(request["source_request"], lock_digest, pymc_worker_digest)
    calibration = obj(
        request["calibration"],
        {
...
def seed_for(seed: int, purpose: str, replicate: int) -> int:
    digest = hashlib.sha256(
        f"marklab-replicated-lgcp-sbc-v1\0{seed}\0{purpose}\0{replicate}".encode()
    ).digest()
...
def simulate(config: dict[str, Any], replicate: int) -> tuple[dict[str, float], np.ndarray]:
    ...
    latent = config["cholesky"] @ rng.normal(0.0, 1.0, len(config["counts"]))
    for pattern in range(len(config["pattern_ids"])):
        selected = config["pattern_index"] == pattern
        latent[selected] -= latent[selected].mean()
    ...
    if int(counts.sum()) > int(config["request"]["resources"]["maximum_total_events"]):
...
def run_sbc(config: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    lgcp = load_module(
        ...
        "marklab_numpyro_replicated_sbc_model",
    )
    kernel = NUTS(
        lgcp.model,
        target_accept_prob=config["target_accept"],
        max_tree_depth=int(config["policy"]["maximum_tree_depth"]),
    )
    ...
    for replicate in range(config["replicates"]):
        try:
            ...
            posterior = az.from_dict(
                {"posterior": {name: samples[name] for name in (
            ...
            maximum_steps = 2 ** int(config["policy"]["maximum_tree_depth"]) - 1
            ...
            draws = {
                ...
                "latent_node": latent[..., config["latent_node_index"]].reshape(-1),
            }
            row: dict[str, Any] = {
                "replicate": replicate,
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"marklab replicated LGCP SBC failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
