#!/usr/bin/env python3
"""Bounded SBC for Marklab's exact NumPyro beta-binomial hierarchy."""
...
REQUEST_FORMAT = "marklab.numpyro_beta_binomial_hierarchy_sbc_worker_request"
RESULT_FORMAT = "marklab.numpyro_beta_binomial_hierarchy_sbc_worker_result"
...
def validate(request: Any, lock_sha: str, worker_sha: str) -> dict[str, Any]:
    ...
    pymc_path = Path(__file__).with_name("marklab_pymc_beta_binomial_hierarchy_worker.py")
    ...
    resources = obj(
        request["resources"],
        {
            "maximum_replicates", "maximum_simulated_trials", "maximum_total_iterations",
...
def seed_for(seed: int, purpose: str, replicate: int) -> int:
    digest = hashlib.sha256(
        f"marklab-beta-binomial-sbc-v1\0{seed}\0{purpose}\0{replicate}".encode()
    ).digest()
...
def run_sbc(config: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for replicate in range(config["replicates"]):
        try:
            ...
            true_population = float(rng.beta(config["population_alpha"], config["population_beta"]))
            true_concentration = float(abs(rng.normal(0.0, config["concentration_sd"])))
            true_patients = rng.beta(
                true_population * true_concentration,
                (1.0 - true_population) * true_concentration,
                patient_count,
            )
            ...
            monitored = {
                "population_probability": samples["population_probability"],
            ...
            if not (
                ...
                continue
            population_draws = samples["population_probability"].reshape(-1)
            ...
            patient_draws = conditional_rng.beta(
                observations[0] + population_draws * concentration_draws,
                trials[0]
                - observations[0]
                + (1.0 - population_draws) * concentration_draws,
            )
            values = [
                ("population_probability", population_draws, true_population),
                ("concentration", concentration_draws, true_concentration),
                ("patient_probability_0", patient_draws, float(true_patients[0])),
            ]
            row: dict[str, Any] = {
                "replicate": replicate,
                "true_population_probability": true_population,
...
def main() -> int:
    ...
    diagnostics = {
        ...
        for name in ["population_probability", "concentration", "patient_probability_0"]
    }
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Marklab beta-binomial SBC worker failed: {type(error).__name__}: {error}",
            file=sys.stderr,
        )
