#!/usr/bin/env python3
"""Static PyMC worker for Marklab's Poisson BYM model."""
...
def finite_vector(value: Any, length: int, path: str) -> np.ndarray:
...
def validate(request: Any, lock_digest: str, worker_digest: str) -> dict[str, Any]:
    ...
    exact(model["family"], "poisson_bym_disease_mapping", "model.family")
    exact(model["likelihood"], "poisson_log_link", "model.likelihood")
    exact(model["offset"], "log_expected", "model.offset")
    exact(
        model["structured_effect"],
        "exact_sum_to_zero_noncentered_icar",
        "model.structured_effect",
    )
    exact(
        model["unstructured_effect"],
        "iid_standard_normal_scaled",
        "model.unstructured_effect",
    )
    ...
    structured_sd_prior = number(model["structured_sd_prior"], "model.structured sd")
    unstructured_sd_prior = number(model["unstructured_sd_prior"], "model.unstructured sd")
    if min(
        intercept_prior_sd,
        coefficient_prior_sd,
        structured_sd_prior,
        ...
        raise ContractError("BYM prior scales must be positive")

    region_ids = request["region_ids"]
    if not isinstance(region_ids, list) or not 6 <= len(region_ids) <= 64:
        raise ContractError("BYM requires 6-64 region IDs")
    if any(
    ...
    rank_deficiency = integer(
        request["rank_deficiency"], "request.rank_deficiency", 1, dimension - 1
    )
    ...
    for component_index, component in enumerate(components):
        if not isinstance(component, list) or len(component) < 2:
            raise ContractError("each component requires at least two regions")
        parsed = [
            integer(value, f"components[{component_index}][]", 0, dimension - 1)
            for value in component
        ]
        if any(value in covered for value in parsed) or len(set(parsed)) != len(parsed):
    ...
    if covered != set(range(dimension)):
        raise ContractError("components must cover every region")
    transform = finite_vector(
        request["icar_transform"], dimension * constrained_dimension, "icar_transform"
    ...
    if any(abs(transform[component, :].sum(axis=0)).max() > 1e-10 for component in parsed_components):
        raise ContractError("ICAR transform violates component sum-to-zero")
    ...
    expected = finite_vector(request["expected"], dimension, "expected")
    ...
    design = finite_vector(request["design"], dimension * predictors, "design").reshape(
    ...
    return {
        ...
        "structured_sd_prior": structured_sd_prior,
        "unstructured_sd_prior": unstructured_sd_prior,
...
def seed_for(seed: int, purpose: str, index: int = 0) -> int:
    digest = hashlib.sha256(f"marklab-pymc-bym-v1\0{seed}\0{purpose}\0{index}".encode()).digest()
...
def fit(config: dict[str, Any], request_sha: str, lock_sha: str, worker_sha: str) -> dict[str, Any]:
    ...
    with pm.Model(coords=coords):
        ...
        structured_sd = pm.HalfNormal("structured_sd", config["structured_sd_prior"])
        unstructured_sd = pm.HalfNormal("unstructured_sd", config["unstructured_sd_prior"])
        ...
        structured = pm.Deterministic(
            "structured", structured_sd * pm.math.dot(config["transform"], structured_raw), dims="region"
        )
        unstructured = pm.Deterministic(
            "unstructured", unstructured_sd * unstructured_raw, dims="region"
        )
        log_relative_risk = pm.Deterministic(
            "log_relative_risk",
            intercept + pm.math.dot(config["design"], coefficient) + structured + unstructured,
            dims="region",
        )
        relative_risk = pm.Deterministic(
            "relative_risk", pm.math.exp(log_relative_risk), dims="region"
        )
        pm.Poisson(
            "observed_count",
    ...
    arrays = {
        ...
        for name in names + ["structured", "unstructured", "relative_risk"]
    }
    ...
    flat_structured = arrays["structured"].reshape(-1, dimension)
    constraints_valid = bool(
        np.all(arrays["structured_sd"] > 0.0)
        and np.all(arrays["unstructured_sd"] > 0.0)
        and all(np.abs(flat_structured[:, component].sum(axis=1)).max() <= 1e-8 for component in config["components"])
    )
    complete = (
        prior_finite
    ...
    flat_unstructured = arrays["unstructured"].reshape(-1, dimension)
    flat_risk = arrays["relative_risk"].reshape(-1, dimension)
    regions = [
        {
            "region_id": region_id,
            "observed_count": int(config["counts"][index]),
            "expected": float(config["expected"][index]),
            "structured_effect": summary(flat_structured[:, index]),
            "unstructured_effect": summary(flat_unstructured[:, index]),
            "relative_risk": summary(flat_risk[:, index]),
        }
    ...
    return {
        "format": "marklab.pymc_bym_worker_result",
        "version": 1,
        "backend": {
            "name": "pymc",
        ...
        "posterior": {
            ...
            "structured_sd": summary(arrays["structured_sd"].reshape(-1)),
            "unstructured_sd": summary(arrays["unstructured_sd"].reshape(-1)),
        },
        "regions": regions,
        "diagnostics": {
