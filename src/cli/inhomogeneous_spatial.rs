...
use crate::{
    ...
    CacheStatus, DurableProject, DurableProjectLimits, InhomogeneousSpatialAnalysisNode,
...
pub(super) fn run_project(request: Request) -> Result<()> {
    ...
    if cells_before != cells_after || window_before != window_after {
        return Err(MarklabError::Validation(
            "inhomogeneous spatial source changed while its durable input was prepared".into(),
        ));
    }
    ...
    eprintln!("project inhomogeneous-spatial cache_status={cache}");
    Ok(())
}
