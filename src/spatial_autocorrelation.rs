...
/// Invalid typed input, design, resource, geometry, or numerical state.
#[derive(Debug, Error, PartialEq)]
pub enum GlobalMoranError {
    /// One or more resource ceilings are zero.
    #[error("global Moran resource limits must be positive")]
    InvalidResourceLimit,
    /// Method-specific inference controls are invalid.
    #[error("invalid global Moran design: {0}")]
    InvalidDesign(String),
    /// Neighbour radius is non-finite or non-positive.
    #[error("global Moran neighbour radius must be finite and positive")]
    InvalidRadius,
    /// Exact observation window has no installed frame binding.
    #[error("global Moran requires a coordinate-frame-bound observation window")]
    UnboundObservationWindow,
    /// Point and window frames differ.
    #[error("global Moran frame mismatch: expected {expected}, observed {observed}")]
    ...
    #[error("global Moran requires at least three points; observed {observed}")]
    InsufficientPoints {
        /// Observed row count.
        observed: usize,
    },
    /// Point rows exceed the explicit limit.
    #[error("global Moran has {observed} points; maximum is {maximum}")]
    PointLimitExceeded {
        /// Observed rows.
        observed: usize,
        /// Explicit maximum.
    ...
    #[error("global Moran point row {row} lies outside the observation window")]
    PointOutsideWindow {
        /// Outside row.
        row: usize,
    },
    /// Two rows have the same physical coordinate and no duplicate policy was declared.
    #[error("global Moran duplicate point rows {first_row} and {second_row}")]
    DuplicatePoint {
        /// First duplicate row.
        first_row: usize,
    ...
    #[error("global Moran requires a typed MarkTable input")]
    TypedMarkTableRequired,
    /// Requested continuous mark is absent.
    #[error("global Moran continuous mark {mark_id:?} is absent")]
    ContinuousMarkMissing {
        /// Missing mark identity.
        mark_id: ScalarMarkId,
    },
    /// An aligned row vector has the wrong length.
    #[error("global Moran row count mismatch: expected {expected}, observed {observed}")]
    RowCountMismatch {
        /// Required rows.
        expected: usize,
        /// Observed rows.
    ...
    #[error("global Moran geometry failed: {0}")]
    Geometry(String),
    /// At least one point has no neighbour under the declared radius.
    #[error("global Moran point row {row} is isolated under the declared weights")]
    IsolatedPoint {
        /// Isolated row.
        row: usize,
    },
    /// Directed edge count exceeds the explicit limit.
    #[error("global Moran directed-edge count exceeds {maximum}")]
    DirectedEdgeLimitExceeded {
        /// Explicit maximum.
        maximum: usize,
    },
    /// Permutation-by-edge work exceeds the explicit limit.
    #[error("global Moran permutation work is {observed}; maximum is {maximum}")]
    ...
    #[error("global Moran histologic_compartment stratum is missing")]
    MissingCompartmentStratum,
    /// No declared stratum contains exchangeable distinct values.
    #[error("global Moran random-labeling null is degenerate")]
    DegenerateNull,
    /// Continuous values have zero variance.
    #[error("global Moran continuous mark has zero variance")]
    ZeroVariance,
    /// Checked count or work arithmetic overflowed.
    #[error("global Moran size arithmetic overflow")]
    SizeOverflow,
    /// Finite inputs produced a non-finite statistic or p-value.
    #[error("global Moran produced a non-finite numerical result")]
    NumericalFailure,
}
