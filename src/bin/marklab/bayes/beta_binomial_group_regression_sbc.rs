...
use marklab_bayes::{
    sha256_hex, NumpyroBetaBinomialGroupSbcWorkerRequest, NumpyroBetaBinomialGroupSbcWorkerResult,
    NutsSamplingSpec,
};
...
pub(super) fn run(
    ...
    let worker_path =
        worker_directory.join("marklab_numpyro_beta_binomial_group_regression_sbc_worker.py");
    let worker_bytes = fs::read(&worker_path).map_err(|source| BayesCliError::Io {
        path: worker_path,
        source,
    })?;
    let request = NumpyroBetaBinomialGroupSbcWorkerRequest::new(
        prepared.request,
        replicates,
    ...
    let result_bytes = run_worker(
        repository,
        "marklab_numpyro_beta_binomial_group_regression_sbc_worker.py",
        &request_bytes,
        timeout_seconds,
    )?;
    let result: NumpyroBetaBinomialGroupSbcWorkerResult = serde_json::from_slice(&result_bytes)?;
