...
use super::super::spatial3d_registered::{
    self, PreparedRegisteredSerialVoxelK, RegisteredSerialVoxelKResult,
};
use super::{
...
const INPUT_KIND: &str =
    "application/vnd.marklab.source.spatial3d-registered-serial-voxel-k+json;version=1";
const OUTPUT_KIND: &str =
    "application/vnd.marklab.spatial3d-registered-serial-voxel-k+json;version=1";
const IMPLEMENTATION_IDENTITY: &str = "marklab-project-spatial3d-registered-serial-voxel-k-node-v1";
...
pub(super) fn run(
    ...
    if !metadata.is_file() || metadata.len() > MAXIMUM_INPUT_BYTES {
        return Err(BayesCliError::Input(
            "registered serial voxel K input must be a regular file within 16 MiB".into(),
        ));
    }
    ...
    let prepared = spatial3d_registered::prepare(&request_bytes)
        .map_err(|error| BayesCliError::Input(error.to_string()))?;
    let after = source_artifact(&input_path, INPUT_KIND)?;
    if before != after {
        return Err(BayesCliError::Input(
            "registered serial voxel K input changed while the durable request was prepared".into(),
        ));
    }

    let node =
        RegisteredSerialProjectNode::new(input_path, before.clone(), prepared, request_bytes)?;
    ...
    let execution = execute_algorithm(
        ...
        &node,
        &scheduler,
        ArtifactSchema::new("marklab.spatial3d_registered_serial_voxel_k", 1)
            .map_err(|error| BayesCliError::Input(error.to_string()))?,
    ...
    eprintln!("project spatial3d-registered-serial-voxel-k cache_status={cache_status}");
    Ok(())
}

struct RegisteredSerialProjectNode {
    spec: NodeSpec,
    input_path: PathBuf,
...
impl RegisteredSerialProjectNode {
    fn new(
        input_path: PathBuf,
        input: ArtifactRef,
        prepared: PreparedRegisteredSerialVoxelK,
        ...
        Ok(Self {
            spec: NodeSpec::new(
                NodeId::new("spatial3d-registered-serial-voxel-k")
                    .map_err(|error| BayesCliError::Input(error.to_string()))?,
                "spatial3d_registered_serial_voxel_k",
                1,
                Vec::new(),
            )
            .map_err(|error| BayesCliError::Input(error.to_string()))?,
...
impl WorkflowNode for RegisteredSerialProjectNode {
    type Output = RegisteredSerialVoxelKResult;
    ...
    fn verify_input_content(&self) -> Result<(), NodeError> {
        ...
        if observed != self.input_artifacts[0] {
            return Err(NodeError::input(BayesCliError::Input(
                "registered serial voxel K input no longer matches its durable identity".into(),
            )));
        }
        Ok(())
    }
    ...
    fn cache_key_material(&self) -> CacheKeyMaterial<'_> {
        CacheKeyMaterial {
            ...
            execution_policy: b"native-safe-rust-registered-serial-voxel-k-v1",
            implementation_identity: IMPLEMENTATION_IDENTITY,
        }
    }

    fn execute(&self) -> Result<Self::Output, NodeError> {
        spatial3d_registered::execute(&self.prepared).map_err(NodeError::execution)
    }
    ...
    fn decode_output(&self, bytes: &[u8]) -> Result<Self::Output, NodeError> {
        let output: RegisteredSerialVoxelKResult =
