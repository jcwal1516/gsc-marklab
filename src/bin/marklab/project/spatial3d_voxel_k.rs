...
use marklab_spatial3d::{voxel_window_k3d, VoxelWindowK3dResult, VoxelWindowK3dSpec};
...
const INPUT_KIND: &str = "application/vnd.marklab.source.spatial3d-voxel-k-function+json;version=1";
const OUTPUT_KIND: &str = "application/vnd.marklab.spatial3d-voxel-k-function+json;version=1";
const IMPLEMENTATION_IDENTITY: &str = "marklab-project-spatial3d-voxel-k-function-node-v1";
...
pub(super) fn run(
    ...
    if !metadata.is_file() || metadata.len() > MAXIMUM_INPUT_BYTES {
        return Err(BayesCliError::Input(
            "spatial3d voxel-window K input must be a regular file within 16 MiB".into(),
        ));
    }
    ...
    let analysis: VoxelWindowK3dSpec = serde_json::from_slice(&request_bytes)?;
    let after = source_artifact(&input_path, INPUT_KIND)?;
    if before != after {
        return Err(BayesCliError::Input(
            "spatial3d voxel-window K input changed while the durable request was prepared".into(),
        ));
    }

    let node =
        Spatial3dVoxelKProjectNode::new(input_path, before.clone(), analysis, request_bytes)?;
    ...
    let execution = execute_algorithm(
        ...
        &node,
        &scheduler,
        ArtifactSchema::new("marklab.spatial3d_voxel_window_homogeneous_k", 1)
            .map_err(|error| BayesCliError::Input(error.to_string()))?,
    ...
    eprintln!("project spatial3d-voxel-k-function cache_status={cache_status}");
    Ok(())
}

struct Spatial3dVoxelKProjectNode {
    spec: NodeSpec,
    input_path: PathBuf,
...
impl Spatial3dVoxelKProjectNode {
    fn new(
        input_path: PathBuf,
        input: ArtifactRef,
        analysis: VoxelWindowK3dSpec,
        ...
        Ok(Self {
            spec: NodeSpec::new(
                NodeId::new("spatial3d-voxel-k-function")
                    .map_err(|error| BayesCliError::Input(error.to_string()))?,
                "spatial3d_voxel_window_homogeneous_k",
                1,
                Vec::new(),
            )
            .map_err(|error| BayesCliError::Input(error.to_string()))?,
...
impl WorkflowNode for Spatial3dVoxelKProjectNode {
    type Output = VoxelWindowK3dResult;
    ...
    fn verify_input_content(&self) -> Result<(), NodeError> {
        ...
        if observed != self.input_artifacts[0] {
            return Err(NodeError::input(BayesCliError::Input(
                "spatial3d voxel-window K input no longer matches its durable identity".into(),
            )));
        }
        Ok(())
    }
    ...
    fn cache_key_material(&self) -> CacheKeyMaterial<'_> {
        CacheKeyMaterial {
            ...
            execution_policy: b"native-safe-rust-bounded-physical-voxel-window-k3d-v1",
            implementation_identity: IMPLEMENTATION_IDENTITY,
        }
    }

    fn execute(&self) -> Result<Self::Output, NodeError> {
        voxel_window_k3d(self.analysis.clone()).map_err(NodeError::execution)
    }
    ...
    fn decode_output(&self, bytes: &[u8]) -> Result<Self::Output, NodeError> {
        let output: VoxelWindowK3dResult =
            serde_json::from_slice(bytes).map_err(NodeError::decode)?;
        output
            .validate_for_spec(&self.analysis)
