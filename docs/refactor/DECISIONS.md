...
## 2026-08-22 — Residual territories reuse contiguous eligible-scale neighborhoods

- Context: The marked detector scanned every cell for each marked candidate at three scales, then repeated the full geometry after cloning the complete `Pattern` for every null label assignment. The multimodal application already had one-index DBSCAN/profile reuse, but the marked spatial stage still built pair and territory geometry independently.
- Decision: The marked spatial stage constructs one `SpatialIndex2D`. `MarkPairCovariancePlan` and `ResidualTerritoryPlan` accept that index explicitly. Residual neighborhoods use one offsets array and one contiguous index array per configuration-eligible scale; each center slice is sorted by original cell index so counts, coordinate sums, component attribution, candidate ordering, and overlap selection match the prior implementation exactly.
- Permutation behavior: The plan is retained through observed detection and all territory-count nulls. Only marks change. Block-mean raster nulls also accept the alternate mark slice directly, deleting the complete `Pattern` clone. Test-only self-indexing constructors remain for independent differential and benchmark use; production cannot silently rebuild an index.
- Evidence: Commit `968d014`; the plan initially failed to compile, then the production reuse regression reproduced 40 builds for observed plus 39 permutations before integration and one afterward. The complete detector matches an independent brute-force oracle across three assignments. Full Nextest passed 343/343 with 15 expected skips before the final eligible-scale pruning; both residual contracts, both affected engine tests, formatting, warnings-denied Clippy, and no-default-features pass afterward.
- Performance: At 256/512/1,024 cells, observed plus 19 nulls improve from estimated repeated-brute totals of 0.620/3.180/12.500 ms to measured plan totals of 0.222/1.086/2.013 ms. Larger radius graph, multimodal territory, and proportional-territory profile workloads through 8,192 points remain below 4× for every doubling; raw ratios and RSS are in `PERFORMANCE_BASELINE.md`.
- Remaining risk: Broad physical scales and large pair radii are output-sensitive and may require large explicit plans. Add configured-budget accounting or a bounded-memory strategy before closing Phase 6; do not infer linear memory from bounded-radius benchmarks.
- Status: Accepted; PERF-01, PERF-05, and PERF-06 fixed. PERF-07 and Phase 6 memory/exit acceptance remain active.

## 2026-08-22 — Filesystem loading is an adapter and million-row input is streamed

- Context: `Pattern::from_paths` made the domain value open files and parse masks. The CSV adapter materialized every owned decoded row before feeding `PatternBuilder`, while the million-cell benchmark separately built the complete fixture in one `String`. Its timing combined decoding and indexed nearest-neighbor finalization under an inaccurate mask-only label.
- Decision: Remove `Pattern::from_paths` and expose `PatternLoader`, which binds an already parsed `TumorMask` and returns a validated `Pattern` with optional `PatternLoadDiagnostics`. CSV decoding invokes a fallible visitor row-by-row, matching the already incremental Parquet path. Rename the combined physical decode/validation/QC/mask/build interval to `decode_and_filter`; retain indexed nearest-neighbor finalization separately.
- Benchmark policy: Stream fixture rows through `BufWriter` before Criterion timing. Keep 10,000 rows as the ordinary smoke profile and exactly 1,000,000 rows only under `MARKLAB_BENCH_PROFILE=full` in scheduled/manual execution. Report full load, internal decode/filter, and internal nearest-neighbor timings; the latter two execute the same complete load but return the authoritative in-memory stage durations to Criterion.
- Evidence: Commit `eb4f5b0`; the public loader contract first failed to compile, the incremental visitor contract failed before the callback API, and the workflow benchmark contract reproduced the giant-`String` source. CSV/Parquet parity and both affected CLI paths pass. Formatting, denied-warning Clippy, no-default-features, doc tests, benchmark build, smoke benchmark, and full Nextest 344/344 with 16 expected skips pass.
- Performance: The manual 1,000,000-row profile completed at 2.548–2.571 seconds total, with 266.35–267.88 ms decode/filter and 2.268–2.275 seconds indexed nearest-neighbor time. `/usr/bin/time -l` recorded 448,856,064 bytes maximum RSS and 23.46 seconds for fixture generation plus repeated benchmark work.
- Compatibility: The Rust API removal and `mask_filter` → `decode_and_filter` timing rename are recorded in the 0.2-to-0.3 migration guide. No compatibility alias keeps filesystem behavior on `Pattern`.
- Status: Accepted; BOUND-01 and PERF-07 fixed. Output-sensitive geometry-plan budget enforcement remains before Phase 6 closure.

## 2026-08-22 — Phase 6 closed; geometry storage is budgeted and Phase 7 opened

- Closure: One deterministic `rstar`-backed `SpatialIndex2D` owns finite immutable points and stable indices. Nearest-neighbor distance, radius/kNN graph, mark-pair covariance, multimodal/marked territories, and profiles use indexed geometry. Application runs build one index per marked or multimodal point set and retain explicit pair/territory plans across label permutations without a monolithic cache.
- Memory decision: Reserve the existing base estimate from `performance.memory_budget_mib`, conservatively estimate the shared R-tree before allocation, and give the remaining bytes to each sequential output-sensitive plan. Pair and residual builders stop before the first stored entry over budget and return a contextual validation error. Telemetry reports base plus `max(index + pair plan, index + residual plan)` because those plans do not coexist.
- Correctness evidence: Brute-force differentials cover nearest, radius, kNN, graph, pair/bin plans, residual territory outputs, multimodal DBSCAN neighbors, and profiles. Duplicate/tie/extreme-coordinate ordering remains deterministic. Separate plan-budget tests plus public 1 MiB rejection and 8 MiB geometry-inclusive telemetry tests pass.
- Performance evidence: Bounded-radius 1,024–8,192 workloads remain below 4× for every doubling; pair and residual geometry is reused for all nulls; post-budget-guard benchmarks show no observed regression. The streamed full 1,000,000-row loader completes in 2.548–2.571 seconds with separately reported decode/filter and indexed-nearest stages. Full raw results, output counts, ratios, RSS, and limitations are in `PERFORMANCE_BASELINE.md`.
- Exit verification: `cargo +1.96.0 fmt --check`, warnings-denied all-target/all-feature Clippy, `cargo +1.96.0 check --locked --no-default-features`, `cargo +1.96.0 nextest run --locked --all-features` (349/349; 16 expected skips), `cargo +1.96.0 test --locked --all-features` (all unit/integration/doc suites green; 15 manual/known library skips and one external WSI fixture skip), `cargo +1.96.0 test --locked --doc --all-features`, and the exact WSI integration command (10/10 local fixtures, one external skip) all exit 0. Dependency policy commands passed when `rstar` was introduced and no dependency changed afterward.
- Remaining risk: The index budget is a conservative representation estimate, and `Vec`/allocator reallocation can transiently differ from retained-byte accounting; Phase 12 must compare it with allocator/RSS profiling. Large configured radii remain output-sensitive and are rejected rather than allowed to exhaust memory. Phase 7 still owns mode-level spectral matrices, chunking, and permutation scratch.
- Phase 7 entry: Reproduce exact consumers of mode-level permutation values, add storage-shape/chunk/determinism differentials, then aggregate into contiguous shell-level rows as early as ERL/scalar semantics permit.
- Status: Accepted; Phase 6 §§13.1–13.8 and PERF-01–07 closed. Phase 7 active.

## 2026-08-22 — Spectrum permutations retain shell rows, not mode matrices

- Context: Binary, probabilistic, and stratified structure-factor paths allocated one `Vec<f64>` of every mode for every permutation. The configured `performance.k_chunk_modes` value was never passed into execution, and ERL immediately collapsed those mode rows into shell curves before inference.
- Decision: Build one immutable `ShellPlan`, aggregate the observed mode vector once, and accumulate permutation power directly into a checked contiguous `B × nonempty-shells` matrix. Process modes in their existing deterministic order in chunks no larger than `k_chunk_modes`, then normalize each shell once. ERL ranks, leave-one-out baselines, scalar summaries, and envelopes consume the flat matrix without constructing nested curve copies.
- Numerical contract: Chunk boundaries do not change addition order within a shell. Every permutation retains its domain-separated index seed, so serial/parallel scheduling and chunk size do not change which labels belong to a permutation. Exact output equality holds for binary, continuous, and stratified paths at chunk sizes 1, 3, and larger than the mode count.
- Trade-off: Bounding phase/mode scratch requires regenerating the same deterministic labels for each mode chunk. Phase 7 must quantify that CPU cost and may adjust the execution plan only if it preserves bounded storage and deterministic identity. Directional anisotropy cannot aggregate to radial shells and remains a separate bounded-chunk task.
- Evidence: Commit `efb8187`; storage shape is observed as `B × shells` and asserted smaller than `B × modes`. ERL oracle, full 254-test all-feature library suite, formatting, warnings-denied all-target/all-feature Clippy, and no-default-features check pass.
- Status: Accepted for primary spectra; PERF-08 remains in progress until anisotropy storage and post-change profiling close.

## 2026-08-22 — Phase 7 closed; spectral storage is bounded by shells and configured chunks

- Closure: Binary, continuous, and stratified structure-factor permutations share one compact mark-field executor and one shell plan. They retain checked contiguous `B × nonempty-shells` matrices rather than `B × modes`. Directional anisotropy cannot be radially aggregated, so it retains one reusable `B × min(k_chunk_modes, directional modes)` matrix, accumulates the observed/null tensors, and discards the chunk.
- Scratch and invariants: Fixed-count labels and a prevalidated `StratifiedPermutationPlan` reuse caller-owned indices, labels, and per-stratum buffers. Binary subset, continuous value, phase-sum, shell, ERL, and anisotropy scratch are bounded and reused at their worker boundary. A local `MarkedAnalysisContext` caches counts, prevalence, and geometry for validation, telemetry, assembly, and anisotropy without making mutable `Pattern` caches possible.
- Determinism and numerical behavior: Chunk sizes 1, typical, and larger than the mode count produce exactly equal binary, continuous, and stratified results. Directional anisotropy exactly matches the retained dense reference for both null strategies. Domain-separated permutation indices remain stable, strict-repeat and one-versus-multiple-thread contracts pass, and the independent GET ERL oracle is unchanged.
- Performance: At 999 permutations, the primary retained matrix falls from 6,361,632 theoretical bytes to 63,936 bytes; same-binary `/usr/bin/time -l` RSS falls from 16,515,072 to 8,896,512 bytes (−46.1%). At the default 256-mode chunk, binary timing changes by −25%/−6%/+2% and continuous by −13%/+1%/+5% at 64/128/256 cells. A tight 64-mode memory cap costs 20–41% at larger small-test cases and is documented rather than hidden. Bounded anisotropy is 4.8–5.3× faster than the exact dense reference.
- Methodology correction: Phase 0 spectrum functions were called outside an engine-owned pool. Although the inherited harness JSON said `thread_count: 1`, `--test-threads=1` controls the Rust test harness, not Rayon; the direct spectral executor used the 12-thread global Rayon pool. Phase 7 comparisons use the same execution context, and the retained Phase 7 metadata now records `executor_threads` explicitly.
- Exit verification: Formatting, warnings-denied all-target/all-feature Clippy, no-default-features, Nextest 358/358 with 19 expected skips, the standard all-feature Cargo suite, doc tests, one/multiple-thread equality, and all three DHAT contracts exit 0. No dependency changed.
- Status: Accepted; PERF-08 and Phase 7 §§14.1–14.10 closed. Phase 8 is active, with multimodal telemetry as its remaining implementation gap.
