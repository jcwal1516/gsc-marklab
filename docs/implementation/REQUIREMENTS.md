...
| FND-04 | Typed marks and measurement provenance | FND-01 | Lead | active | Infrastructure only | C-06 adds the exact four-state measurement-origin vocabulary, one immediate patch computation, and one CellId/frame/provenance-bound binary/optional-probability engine/project workflow with exact threshold evidence; general mark kinds, units/modalities, missingness, durable formats, and adapters remain pending |
| FND-05 | Cell/patch embedding artifacts | FND-01, FND-04 | Lead | active | No model-specific claim | C-04 cell and C-05 multiscale storage/link substrates plus one measurement-declared descriptive patch caller are complete; real-source correspondence, geometry/window truth, and formal embedding science remain prerequisites for broader closure |
...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Two vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion, then a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; general marks remain pending |
