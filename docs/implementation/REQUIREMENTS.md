...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Eight vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; bounded declared patch-to-finalized-region aggregation dispersion; a runtime-only semantic pre/post workflow; exact descriptive binary-prevalence change over existing scheduler outputs; lineage-exact discrepancy between both completed slide aggregation paths; exact binary-group cell-embedding centroid discrepancy; and dense-probability cell-embedding cross-covariance energy. General marks and durable producer proof remain pending |
...
| WS-23 | active | C-06 supplies measurement status, one exact declared binary/optional-probability engine/project workflow, semantic pre/post comparison, binary-prevalence change, and verified cell-embedding consumers for both binary assignments and dense probabilities; general typed marks, formats, missingness, durable producer proof, and adapters remain |
...
| WS-50 | active/two bounded synthetic slices | Exact declared binary-group centroid discrepancy and dense-probability population cross-covariance energy over verified present cell embeddings are complete; spatial covariance/variograms, graph/kernel/local-diversity/boundary/retrieval/cross-sample designs, real-source promotion, and inferential validation remain planned/data-dependent |
