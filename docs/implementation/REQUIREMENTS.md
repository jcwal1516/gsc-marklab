...
| WS-50 | active/four bounded synthetic slices | Exact declared binary-group centroid discrepancy, dense-probability and concrete nucleus-area population cross-covariance energies, and contained-patch cell-embedding local dispersion are complete; the centroid question also has one semantic-store-verified cacheable project execution. Spatial covariance/variograms, graph/kernel/boundary/retrieval/cross-sample designs, real-source promotion, and inferential validation remain planned/data-dependent |
