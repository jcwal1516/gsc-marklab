...
| FND-05 | Cell/patch embedding artifacts | FND-01, FND-04 | Lead | active | No model-specific claim | C-04 cell and C-05 multiscale storage/link substrates plus bounded binary-group centroid, probability and nucleus-area cross-covariance, patch-overlap, patch-to-derived-region, and exact slide aggregation-path descriptive callers are complete; real-source correspondence, geometry/window truth, and formal spatial embedding science remain prerequisites for broader closure |
...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Ten vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; bounded declared patch-to-finalized-region aggregation dispersion; a runtime-only semantic pre/post workflow; exact descriptive binary-prevalence change over existing scheduler outputs; lineage-exact discrepancy between both completed slide aggregation paths; exact binary-group cell-embedding centroid discrepancy; dense-probability and concrete nucleus-area cell-embedding cross-covariance energies; and one semantic-store-verified cacheable project execution of the exact centroid question. General marks and durable producer proof remain pending |
...
| WS-50 | active/three bounded synthetic slices | Exact declared binary-group centroid discrepancy plus dense-probability and concrete nucleus-area population cross-covariance energies over verified present cell embeddings are complete; the centroid question also has one semantic-store-verified cacheable project execution. Spatial covariance/variograms, graph/kernel/local-diversity/boundary/retrieval/cross-sample designs, real-source promotion, and inferential validation remain planned/data-dependent |
