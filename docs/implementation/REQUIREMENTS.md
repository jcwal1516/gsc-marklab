...
| FND-04 | Typed marks and measurement provenance | FND-01 | Lead | active | Infrastructure only | C-06 adds the exact four-state measurement-origin vocabulary, two immediate embedding computations, and one CellId/frame/provenance-bound binary/optional-probability engine/project workflow with exact threshold evidence; general mark kinds, units/modalities, missingness, durable formats, and adapters remain pending |
...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Three vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; and bounded declared patch-to-finalized-region aggregation dispersion. General marks remain pending |
