...
| FND-04 | Typed marks and measurement provenance | FND-01 | Lead | active | Infrastructure only | C-06 adds the exact four-state measurement-origin vocabulary, four immediate embedding computations including one exact binary-mark-conditioned cell-centroid diagnostic, one CellId/frame/provenance-bound binary/optional-probability engine/project workflow with exact threshold evidence, one semantic declared pre/post workflow, and exact descriptive binary-prevalence change retaining both contexts; general mark kinds, units/modalities, missingness, durable formats, producer proof, and adapters remain pending |
| FND-05 | Cell/patch embedding artifacts | FND-01, FND-04 | Lead | active | No model-specific claim | C-04 cell and C-05 multiscale storage/link substrates plus bounded binary-group cell-centroid, patch-overlap, patch-to-derived-region, and exact slide aggregation-path descriptive callers are complete; real-source correspondence, geometry/window truth, and formal spatial embedding science remain prerequisites for broader closure |
...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Seven vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; bounded declared patch-to-finalized-region aggregation dispersion; a runtime-only semantic pre/post workflow; exact descriptive binary-prevalence change over existing scheduler outputs; lineage-exact discrepancy between both completed slide aggregation paths; and exact binary-group cell-embedding centroid discrepancy over a verified table. General marks and durable producer proof remain pending |
...
| WS-23 | active | C-06 supplies measurement status, one exact declared binary/optional-probability engine/project workflow, semantic pre/post comparison, binary-prevalence change, and a binary-mark-conditioned verified cell-embedding centroid diagnostic; general typed marks, formats, missingness, durable producer proof, and adapters remain |
| WS-24 | active/data available with blockers | C-04/C-05 embedding infrastructure and C-06's bounded descriptive callers, including binary-group cell centroids and exact cross-path slide discrepancy, are complete. Authorized candidates still lack promotable canonical identity/context/provenance/linkage; region/slide source profiles remain inadmissible |
...
| WS-50 | active/bounded synthetic slice | Exact declared binary-group centroid discrepancy over verified present cell embeddings is complete; spatial covariance/variograms, graph/kernel/local-diversity/boundary/retrieval/cross-sample designs, real-source promotion, and inferential validation remain planned/data-dependent |
