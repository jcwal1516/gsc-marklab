...
|---|---|---|---|---|---|---|
| PLAT-01 | Project, artifact, and workflow operating system | WS-A baseline | Lead | active | New project/workflow surface | B-04 project/store/cache slice plus one semantic-store-verified C-06 scientific project execution complete; durable project heads/execution ledger/resume remain |
| WF-01 | Typed composable workflow DAG | PLAT-01 | Lead | active | New workflow schema/API | B-04 single-node typed slice and one concrete C-06 scientific node/codec execution complete; general composition/resume remains |
| BACK-01 | Versioned backend registry | PLAT-01, WF-01 | Lead | planned | Backend manifests/CLI | Pending |
| DATA-01 | Identity, hierarchy, modalities, coordinates, and units | WS-B | Lead | active | New stable data contracts | C-01 identity/hierarchy, C-02 coordinates, C-03 artifacts, C-04/C-05 embeddings, and C-06's borrowed declared scalar-pattern identity are complete; broader durable payload alignment remains |
...
| EMB-PATCH | Patch/region/slide embeddings and links | DATA-01, artifact layer | Lead | complete | New artifact contract | C-05 closes typed logical/record/graph/receipt contracts, all eight exact Arrow/Parquet families, bounded fuzz/differential evidence, shared-vector DHAT, and checksum-pinned 10k/100k-patch scale gates; no real source profile is promoted |
...
| FND-04 | Typed marks and measurement provenance | FND-01 | Lead | active | Infrastructure only | C-06 adds the exact four-state measurement-origin vocabulary, seven immediate embedding computations, whole-input and contained-patch binary-group nucleus-area contrasts, one CellId/frame/provenance-bound binary/optional-probability engine/project workflow with exact threshold evidence, a dedicated cacheable project execution for the exact binary-centroid question, one semantic declared pre/post workflow, and exact descriptive binary-prevalence change retaining both contexts; general mark kinds, arbitrary units/modalities, missingness, durable formats, producer proof, and adapters remain pending |
| FND-05 | Cell/patch embedding artifacts | FND-01, FND-04 | Lead | active | No model-specific claim | C-04 cell and C-05 multiscale storage/link substrates plus bounded binary-group centroid, probability and nucleus-area cross-covariance, contained-patch cell local dispersion, patch-overlap, patch-to-derived-region, and exact slide aggregation-path descriptive callers are complete; real-source correspondence, geometry/window truth, and formal spatial embedding science remain prerequisites for broader closure |
...
| CMP-01 | Spatial fingerprints/two-sample tests | COH-01 | Lead | planned | Descriptive current comparison only | C-06 now exposes semantic pre/post spatial summaries plus binary marked-row prevalence change; no population two-sample test or biological-unit design is claimed |
...
| C-05 | Patch/region/slide embedding links | C-02–C-04 | Lead | complete | Data/artifact tests | Commits `4e1d5b5`–`966acaa` plus the self-referential closure commit; complete typed/record/graph/receipt and eight-family physical contracts, deterministic region/both-slide finalization, bounded fuzz/differential evidence, 10k/100k patch scale, DHAT, RSS, and phase gates; `handoffs/C-05.md` |
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Thirteen vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; bounded declared patch-to-finalized-region aggregation dispersion; a runtime-only semantic pre/post workflow; exact descriptive binary-prevalence change over existing scheduler outputs; lineage-exact discrepancy between both completed slide aggregation paths; exact binary-group cell-embedding centroid discrepancy; dense-probability and concrete nucleus-area cell-embedding cross-covariance energies; one semantic-store-verified cacheable project execution of the centroid question; exact contained-patch cell-embedding local dispersion over the completed C-04→C-05 graph/receipt boundary; whole-input binary-group nucleus-area contrast; and equal-patch contained-local nucleus-area contrast. General marks, source correspondence, and durable result provenance remain pending |
...
| WS-11 | active | B-04 reference/digest/cache, C-03 immutable catalog/local store, and one C-06 semantic scientific cache execution are implemented and green; mutable project heads, durable execution ledgers, and resume remain |
| WS-12 | active | B-04 typed DAG/key/failure-atomic single-node scheduler plus one concrete C-06 scientific node/private codec are implemented and green; general scheduling/resume follows |
...
| WS-23 | active | C-06 supplies measurement status, one exact declared binary/optional-probability engine/project workflow, a dedicated project execution of the binary-centroid question, semantic pre/post comparison, binary-prevalence change, one fixed nucleus-area morphology declaration, whole-input and contained-patch binary-group nucleus-area contrasts, and verified cell-embedding consumers for binary assignments, dense probabilities, and nucleus area; general typed marks, formats, missingness, durable producer proof, and adapters remain |
| WS-24 | active/data available with blockers | C-04/C-05 embedding infrastructure and C-06's bounded descriptive callers, including binary centroids, probability and nucleus-area cross-covariance, contained-patch cell local dispersion, and exact cross-path slide discrepancy, are complete. Authorized candidates still lack promotable canonical identity/context/provenance/linkage; region/slide source profiles remain inadmissible |
...
| WS-50 | active/five bounded synthetic slices | Exact declared binary-group centroid discrepancy, dense-probability and concrete nucleus-area population cross-covariance energies, contained-patch cell-embedding local dispersion, and direct binary-group nucleus-area contrast are complete; the centroid question also has one semantic-store-verified cacheable project execution. Spatial covariance/variograms, graph/kernel/boundary/retrieval/cross-sample designs, real-source promotion, and inferential validation remain planned/data-dependent |
| WS-51 | active/two bounded synthetic slices | The completed contained-shared cell-patch graph and paired physical receipt now have descriptive cell-embedding local-dispersion and equal-patch binary-group nucleus-area consumers. Cell/patch vector complementarity, interpolation semantics, source correspondence, real-source promotion, multiscale kernels/fingerprints, and overlap-aware inference remain planned/data-dependent |
