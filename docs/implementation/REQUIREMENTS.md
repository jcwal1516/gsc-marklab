...
| FND-04 | Typed marks and measurement provenance | FND-01 | Lead | active | Infrastructure only | C-06 adds the exact four-state measurement-origin vocabulary, three immediate embedding computations, one CellId/frame/provenance-bound binary/optional-probability engine/project workflow with exact threshold evidence, one semantic declared pre/post workflow, and exact descriptive binary-prevalence change retaining both contexts; general mark kinds, units/modalities, missingness, durable formats, producer proof, and adapters remain pending |
| FND-05 | Cell/patch embedding artifacts | FND-01, FND-04 | Lead | active | No model-specific claim | C-04 cell and C-05 multiscale storage/link substrates plus bounded patch-overlap, patch-to-derived-region, and exact slide aggregation-path descriptive callers are complete; real-source correspondence, geometry/window truth, and formal embedding science remain prerequisites for broader closure |
...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Six vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; bounded declared patch-to-finalized-region aggregation dispersion; a runtime-only semantic pre/post workflow; exact descriptive binary-prevalence change over existing scheduler outputs; and lineage-exact discrepancy between both completed slide aggregation paths. General marks and durable producer proof remain pending |
...
| WS-23 | active | C-06 supplies measurement status, one exact declared binary/optional-probability engine/project workflow, semantic pre/post comparison, and binary-prevalence change; general typed marks, formats, missingness, durable producer proof, and adapters remain |
| WS-24 | active/data available with blockers | C-04/C-05 embedding infrastructure and C-06's bounded descriptive callers, including exact cross-path slide discrepancy, are complete. Authorized candidates still lack promotable canonical identity/context/provenance/linkage; region/slide source profiles remain inadmissible |
