...
| FND-04 | Typed marks and measurement provenance | FND-01 | Lead | active | Infrastructure only | C-06 adds the exact four-state measurement-origin vocabulary, two immediate embedding computations, one CellId/frame/provenance-bound binary/optional-probability engine/project workflow with exact threshold evidence, and one semantic declared pre/post workflow retaining both runtime identities/evidence chains; general mark kinds, units/modalities, missingness, durable formats, producer proof, and adapters remain pending |
...
| C-06 | General marks/status | C-01, C-03 | Lead | active | Data/artifact tests | Four vertical slices pass: four provenance statuses plus bounded patch-overlap dispersion; a borrowed declared scalar-pattern direct/project workflow with compact runtime identity, exact provenance/threshold binding, legacy numerical parity, and unchanged result-0.3 bytes; bounded declared patch-to-finalized-region aggregation dispersion; and a runtime-only semantic pre/post workflow over existing declared scheduler outputs. General marks and durable producer proof remain pending |
...
| WS-23 | active | C-06 supplies measurement status and one exact declared binary/optional-probability engine/project workflow; general typed marks, formats, missingness, and adapters remain |
