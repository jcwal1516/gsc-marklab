...
| WS-23 | active | C-06 supplies measurement status, one exact declared binary/optional-probability engine/project workflow, a dedicated project execution of the binary-centroid question, semantic pre/post comparison, binary-prevalence change, one fixed nucleus-area morphology declaration, a direct binary-group nucleus-area contrast, and verified cell-embedding consumers for binary assignments, dense probabilities, and nucleus area; general typed marks, formats, missingness, durable producer proof, and adapters remain |
...
| WS-51 | active/one bounded synthetic slice | The completed contained-shared cell-patch graph and paired physical receipt now have one descriptive cell-embedding local-dispersion consumer. Cell/patch vector complementarity, interpolation semantics, source correspondence, real-source promotion, multiscale kernels/fingerprints, and overlap-aware inference remain planned/data-dependent |
