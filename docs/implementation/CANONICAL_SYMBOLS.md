...
| C-05 direct-patch runtime receipts | `crates/marklab-embeddings/src/multiscale/matrix_artifact.rs`, `src/multiscale/records/artifact_graph/{error,mod}.rs`, Arrow/Parquet matrix readers | `VerifiedPatchEmbeddingSupportArtifact` composes exact graph-bound footprint/overlap receipts across physical formats; `VerifiedPatchEmbeddingTableArtifact` requires full physical validation plus graph-bound source-row PatchId/status equality, expected/support/provenance logical identity, dimension, QC, and distinct dependencies; opaque source components remain unproved | Four borrowed/managed Arrow/Parquet receipt entry points; future derived-region graph | `tests/multiscale_embedding_artifact_graph/matrix_receipts.rs` | C-05 |
