...
## DEC-0305 — Reuse pinned witness persistence for one admitted CellViT coordinate caller

- Date: 2026-08-28
- Status: accepted for TOP-01/TOP-01A/WF-01/WS-12/WS-63
- Decision: add `marklab project witness-persistence` and one CellViT adapter request around the existing GUDHI 3.13.0 witness worker. Retain exact source CellIds and micrometre coordinates; use deterministic farthest-point selection with at most 64 landmarks, dimensions 0–2, `nu=0`, a prespecified 200-micrometre maximum scale, field 2, 500,000-simplex ceiling, and 180-second timeout. Split the existing direct command into prepare/execute phases so the durable node can bind canonical request bytes containing GUDHI/Python/license/environment/worker identity, reuse the existing scheduler/ledger/store/runtime identity, encode artifacts through the existing exact-f64 codec, and revalidate decoded hits through the same typed result contract. Expose only the result validation method immediately required by that durable decoder.
- Consequences: one real 2,000-cell pathology coordinate sample now has bounded direct execution, exact typed durable replay, and an analytic five-point oracle without a topology registry or second backend mechanism. The result remains a single-slide descriptive witness approximation; this does not establish patient/cohort replication, subsampling or segmentation stability, a molecular endpoint, optimal landmarks/scale, exact full-complex topology, or biological significance.

## DEC-0306 — Admit weighted arbitrary-window IPP likelihoods before backend fitting

- Date: 2026-08-28
- Status: accepted for BAY-PP/WF-01/WS-12/WS-43
- Decision: add `arbitrary_window_ipp_log_likelihood`, `marklab bayes arbitrary-window-ipp-likelihood`, and the matching durable project path as the minimum caller that removes the rectangle-only likelihood boundary. Reuse the exact `ObservationWindow2D` MultiPolygon owner. Require unique finite in-window events, unique finite in-window quadrature representatives, positive supplied cell-intersection weights whose compensated sum agrees with exact window area within `1e-10` relative tolerance, one fixed covariate plus offset at events and nodes, fixed intercept/coefficient, and explicit event/node/work/retained-byte ceilings. Persist canonical event/quadrature/window identities, assumptions, statistical unit, lack of inferential null, finite-result policy, and exact typed durable replay. Use a dedicated CLI parser because adding the command to the legacy monolithic Bayes enum crosses its observed main-thread stack ceiling.
- Consequences: holes and disconnected tissue components enter one real point-process likelihood without replacing them by a bounding rectangle or changing the pinned rectangular PyMC worker. The CellViT adapter computes weights from exact Shapely intersections of a fixed 64-by-64 bounding grid and records the rule. This does not fit coefficients, establish quadrature convergence for a varying field, add posterior uncertainty/PPC, infer interaction, treat cells as population replicates, generalize window formats, or create a point-process plugin system; a pinned-backend fit is the next caller.

## DEC-0307 — Fit weighted exact-window IPP through the unchanged pinned worker

- Date: 2026-08-28
- Status: accepted for BAY-PP/BACK-01/WF-01/WS-13/WS-40/WS-43
- Decision: add `fit-arbitrary-window-ipp` direct and durable commands without copying or changing the pinned PyMC rectangular IPP worker. Canonically order the exact-window events/nodes, map `m` positive quadrature nodes to an algebraic `m`-by-1 equal-area backend grid, preserve event covariate/offset values, and add `ln(weight_um2)` to each node offset so the unchanged worker's unit-cell integral equals the exact weighted integral. Treat backend cell coordinates and observed-cell assignments as adapter internals; publish only restored exact physical node identity/coordinates/weights, intensity per square micrometre, expected count, total predictive summaries, posterior coefficient/intercept, diagnostics, exact outer input/window identities, and backend request identity. Restrict this adapter to 4–1,024 nodes, 20–100,000 events, explicit draw-node work, existing NUTS/time/output limits, and one fixed covariate plus offset.
- Consequences: the immediate arbitrary-window caller gains posterior uncertainty and durable execution while preserving the proven static environment/worker and its cache identity. Separate seeded executions can differ at floating-reduction scale, so only durable replay is byte-exact; independent real executions are reported with their measured numeric tolerance. This does not make synthetic backend coordinates physical, expose backend per-cell residuals, fit interactions/marks/latent fields, establish quadrature sensitivity or calibration, claim population inference from one pattern, or create a generalized point-process adapter registry.

## DEC-0308 — Promote weighted IPP with one exact NumPyro caller and fixed prior grid

- Date: 2026-08-28
- Status: accepted for BAY-02/BAY-PP/BACK-01/WS-40/WS-43/WS-44
- Decision: add one NumPyro 0.21.0/JAX 0.11.1 worker for the immediate weighted-IPP agreement caller. The worker imports the existing PyMC IPP validator, consumes that exact transformed request inside a wrapper with its own backend/worker identity, and fits only intercept and one coefficient through the same event-sum minus weighted-integral potential. Compare PyMC/NumPyro intercept, coefficient, and total expected-count posterior means with caller-declared positive absolute tolerances and parameter interval overlap; report disagreement rather than selecting a backend. Add a separate five-fit PyMC prior sensitivity command varying only intercept- and coefficient-prior SDs by 0.5x/2x around baseline, using the same data/seed/sampling and a caller-declared standardized materiality threshold under a 100-million total draw-node ceiling.
- Consequences: the exact-window fit gains independent implementation and bounded prior-scale evidence without a backend registry, copied PyMC validator, new environment, or adaptive scenario search. This does not constitute SBC, quadrature convergence, spatial PPC, CmdStan/GPU evidence, exhaustive prior robustness, or patient-level validation; those remain required before family promotion.

## DEC-0309 — Promote the weighted IPP with bounded calibration, refinement, and spatial checks

- Date: 2026-08-28
- Status: accepted for BAY-02/BAY-PP/BACK-01/WS-40/WS-43/WS-44
- Decision: add three concrete diagnostics around the DEC-0307 likelihood. Compare the same fit over fixed 48/64/80 exact-area quadrature grids and retain every resolution's exact backend, environment, worker, request, input, and sampling identity. Run 20–100 prior-generative NumPyro SBC replicates for intercept, coefficient, and total expected count under explicit generated-count and draw-node ceilings, retaining failed prior draws rather than discarding or replacing them. For spatial PPC, add one exact `event_id`→64-by-64 quadrature-node interchange table, build a fixed physical-radius graph over exact node representatives, and compare observed versus posterior-replicated node-density variance and neighbor density contrast. Extract the existing PyMC sampler into a callable function inside its unchanged static worker file so the spatial diagnostic reuses the identical validator, likelihood, priors, and sampling implementation; the worker digest and durable cache key therefore change exactly once. Add exact backend/request/input identities to the already-live prior-sensitivity output as additive fields.
- Consequences: the real CellViT exact-window model now has backend agreement, prior sensitivity, quadrature refinement, prior-generative calibration, spatial posterior-predictive diagnostics, and refreshed durable replay without a model registry or copied likelihood. The deployed diffuse prior is not calibration-accepted because four of 20 deterministic draws imply more than ten million cells; a separately named physically scaled prior passes 20/20, so the limitation is prior support rather than hidden. The fitted nonspatial x-gradient also under-replicates observed local variation and is not promoted as an adequate spatial biological model. This does not tune toward significance, add a latent spatial field, infer event membership, treat quadrature nodes as population replicates, claim patient-level inference, or create a generic diagnostic framework.

## DEC-0310 — Reuse the pinned dense LGCP for one coarse exact-window latent field

- Date: 2026-08-28
- Status: accepted for BAY-PP/BAY-PP-A/BACK-01/WF-01/WS-13/WS-43/WS-44
- Decision: add direct and durable `fit-arbitrary-window-lgcp` for the immediate admitted CellViT window by wrapping the existing pinned PyMC gridded-LGCP worker. Canonically require 4–36 positive exact-window quadrature nodes and one complete exact event-to-node membership row per event. Map weights through the existing equal-area/log-weight algebraic adapter, then replace only the source worker's synthetic-grid Cholesky factor with a Rust-constructed fixed Matérn-3/2 covariance over the exact physical node representatives. Retain exact outer/source backend, lock, worker, request, input, window, covariance, sampling, seed, work, prediction, and process identities; reuse the existing scheduler, ledger, artifact store, output transaction, and exact-f64 codec for durable replay. Add physical-radius node-density PPC and one prespecified half/base/double amplitude/length-scale sensitivity caller under explicit node, event, neighbor, draw-node, predictive-point, output, and timeout ceilings.
- Consequences: one exact MultiPolygon CellViT caller gains a bounded latent spatial intensity field without a second likelihood, environment, backend registry, mesh framework, or result-format change. The 8-by-8 clipped bounding-grid partition is a recorded coarse approximation, kernel hyperparameters are fixed rather than inferred, and one observed point pattern remains the statistical unit. The real fit's negative spatial PPC precludes adequacy or attraction claims even though the fixed-kernel grid is below its prespecified materiality threshold. This does not establish quadrature/component stability, calibrate field hyperparameters, fit event interaction or marks, create independent patient replication, or provide biological, causal, clinical, or external-validation evidence.

## DEC-0311 — Promote the exact-window field through existing NumPyro and fixed partitions

- Date: 2026-08-28
- Status: accepted for BAY-02/BAY-PP/BAY-PP-A/BACK-01/WS-40/WS-43/WS-44
- Decision: add three concrete promotion callers around DEC-0310. For agreement, validate the exact PyMC arbitrary-window request, override the existing NumPyro gridded-LGCP configuration with that already-validated physical Cholesky factor, and compare global parameters plus every physical latent and expected-count node with the existing Monte Carlo-aware rules. For SBC, validate the same physical request and call the existing NumPyro gridded-LGCP SBC implementation with its physical Cholesky, retaining every 20–100 replicate disposition, source/adapter identity, and prespecified physical latent node. For resolution sensitivity, fit exact 4/6/8 bounding-grid partitions, compare global posterior shifts and total expected counts to the deployed 8-grid baseline, and retain each resolution's spatial PPC without treating aggregation-dependent node-density summaries as interchangeable. Use only the existing Python 3.12 lock, PyMC/NumPyro workers, likelihoods, samplers, diagnostics, and fixed result limits.
- Consequences: the fixed-kernel exact-window CellViT field gains independent-backend, calibration, and quadrature evidence without a sampler copy, new environment, registry, adaptive mesh, hyperparameter search, or result-format change. Its global posterior passes the declared checks, but the 32-node spatial PPC remains inadequate and one slide remains one observed pattern. This does not infer kernel hyperparameters, calibrate adaptive refinement, establish point interaction or attraction, fit multitype/marked effects, or create independent patient-population evidence.

## DEC-0312 — Fit exact slide patterns as repeated measurements within patients

- Date: 2026-08-28
- Status: accepted for BAY-03/BAY-PP/BACK-01/WF-01/WS-13/WS-41/WS-43
- Decision: add direct and durable `fit-replicated-arbitrary-window-lgcp` for one balanced immediate CPTAC caller. Deterministically select four provenance-complete repeated-slide patients per MSI/MSS group and two slides per patient. Stream every selected slide's exact source CellIds into an event digest and exact 4-by-4 clipped-node counts without emitting an unbounded membership table. Fit one shared intercept, MSI-versus-MSS and within-slide x effects, unconstrained zero-mean patient random intercepts, slide effects constrained to sum zero within patient, and fixed Matérn-3/2 latent fields constrained to sum zero within slide. Keep every exact window/event/node identity separate; never concatenate patterns or treat slides, nodes, or cells as patient replicates. Reuse the pinned PyMC environment and existing durable scheduler/ledger/store/exact-f64 transaction with explicit patient/pattern/node/event/draw/time/output ceilings and 500 deterministic prior-finiteness draws.
- Consequences: a real exact-window field now estimates patient-population uncertainty across repeated slide patterns instead of stopping at one descriptive slide. Group-wise centering of patient effects is forbidden because it conditions away between-patient group uncertainty; an attempted diagnostic using it was rejected. The retained eight-patient result is null-compatible and exploratory. This does not establish cohort transportability, infer field hyperparameters, fit cohort/site effects, provide cross-backend/SBC/sensitivity/PPC promotion, or support causal, clinical, or significance claims.

## DEC-0313 — Promote the replicated field without hiding kernel or prior failures

- Date: 2026-08-29
- Status: accepted for BAY-03/BAY-PP/BAY-PP-A/BACK-01/WS-41/WS-43/WS-44
- Decision: extend the immediate DEC-0312 caller with node posterior summaries and deterministic per-slide total-count and node-variance posterior-predictive checks, changing its typed result and durable node identity to version 2. Add one NumPyro 0.21.0/JAX 0.11.1 agreement worker over the exact validated request, with an explicit sampler-specific tree-depth capacity, and compare every global, patient, slide, latent-node, and expected-count posterior under Monte Carlo-aware tolerances. Add a fixed nine-fit PyMC grid that varies patient SD, slide SD, field amplitude, and field length scale one at a time by 0.5x/2x under one aggregate work ceiling. Add 20–100 prior-generative NumPyro SBC replicates for group effect, patient SD, slide SD, and one prespecified physical latent node, retaining every failed or over-ceiling replicate.
- Consequences: the real model now has exact durable PPC, independent-backend agreement, hierarchy/kernel sensitivity, and real-geometry calibration evidence without a model registry, new environment, or generic diagnostic framework. Depth-10 and depth-12 NumPyro agreement attempts remain rejected; depth 13 is complete. The deployed diffuse prior is not calibration-accepted because 14/20 draws exceed its event ceiling and two admitted fits saturate tree depth, while a separately named feasible count-scale prior passes 20/20. The real group effect is backend-consistent and remains null-compatible, but the fixed spatial decomposition is materially kernel-sensitive and three sensitivity scenarios are nonconverged. Therefore this caller is not promoted as a stable biological spatial explanation and does not establish attraction, transportability, causality, clinical utility, or significance.

## DEC-0314 — Infer one shared physical kernel for the immediate replicated caller

- Date: 2026-08-29
- Status: accepted for BAY-03/BAY-04/BAY-PP/BAY-PP-A/BACK-01/WF-01/WS-13/WS-41/WS-43
- Decision: address DEC-0313's material fixed-kernel sensitivity with one concrete PyMC caller over the unchanged exact patient/pattern/window request. Infer a positive shared isotropic Matérn-3/2 amplitude and micrometre length scale, retaining the same group, patient, slide, covariate, and zero-sum field hierarchy. Construct symbolic Cholesky factors separately for each 4–16-node slide block rather than one dense 222-node covariance. Record amplitude/length priors, an explicit 10–14 tree-depth capacity, retained draw-node work, sum-of-pattern-cubes factorization work, wall time, source worker, inferred worker, lock, request, input, result, and durable cache identities. Reuse the existing scheduler, ledger, artifact store, transaction, and exact-f64 codec.
- Consequences: the real repeated-pattern field no longer selects one fixed kernel post hoc. At depth 13 it is diagnostically complete and estimates amplitude 0.619 [0.536, 0.719] and length scale 335 micrometres [251, 435], while the group effect remains null-compatible and slide predictive checks improve. The depth-10 fit with 377 ceiling hits remains a rejected diagnostic. This bounded shared stationary kernel does not establish patient-specific/nonstationary/anisotropic scales, attraction, calibration, backend agreement, transportability, causality, clinical utility, or biological significance.

## DEC-0315 — Test the inferred replicated kernel without relaxing failed calibration

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-04/BAY-PP/BAY-PP-A/BACK-01/WS-40/WS-43/WS-44
- Decision: add one independent NumPyro 0.21.0/JAX 0.11.1 implementation of DEC-0314's exact dynamic block-Cholesky Matérn hierarchy and compare all seven global parameters, every patient/slide effect, and every physical latent/expected-count node with the existing Monte Carlo-aware agreement rules. Add one prior-generative SBC caller for group effect, patient/slide scales, inferred amplitude, inferred micrometre length, and one prespecified physical latent node. Require 20–100 exact replicate dispositions, 90% coverage and rank-uniformity bounds, the unchanged strict sampler diagnostics, and explicit node/iteration/event/kernel/time/output ceilings. Retain failed replicates and permit only a declared sampling-capacity retry that leaves seed, priors, geometry, and acceptance thresholds unchanged.
- Consequences: the real eight-patient/16-slide fit is independently backend-consistent, and the 20-replicate synthetic oracle calibrates all six SBC quantities. Real 222-node count-scale SBC is not promoted: the first run completes 19/20 because one R-hat is 1.01048 against 1.01, and the single stronger-capacity retry completes 19/20 because a different replicate has one divergence. Both negative results remain sealed. This does not authorize threshold relaxation, seed search, post-hoc prior tuning, a generic calibration framework, attraction or significance claims, patient-specific kernels, transportability, causality, or clinical utility.

## DEC-0316 — Fit hard multitype marks conditionally on the admitted location graph

- Date: 2026-08-29
- Status: accepted for BAY-01/BAY-PP/BACK-01/FND-04/MRK-02A/WF-01/WS-13/WS-23/WS-43
- Decision: add direct and durable `fit-conditional-multitype-mark` for one exact fixed-location physical-radius graph with 3–8 complete hard classes. Fit a symmetric categorical Gibbs single-site conditional pseudolikelihood in pinned PyMC: reference-type intercept zero and reference/reference potential zero identify the gauge; every other intercept and unordered pair potential receives a declared Normal prior. Report gauge-invariant cross-versus-average-homotypic affinity contrasts, intercept-only independent-categorical-label composite score, and one-step conditional type-count/same-edge expectations given observed neighbors. Bind exact source/backend/environment/worker/request/radius/type/edge/sampling identities and cap points, types, pair visits, retained edges, draw-parameter work, retained working bytes, request/output bytes, tree depth, and time. Reuse the existing durable scheduler, ledger, artifact store, transaction, and exact-f64 codec.
- Consequences: the admitted four-class CellViT table gains a fitted uncertainty-bearing hard-multitype workflow without collapsing to Neoplastic versus all others. The real single-slide fit is diagnostically complete and all six cross-versus-homotypic intervals are negative, but the location process is conditioned on, the pseudolikelihood is not a normalized joint Gibbs likelihood, the intercept-only null does not preserve exact class counts, one-step expectations are not joint posterior simulations, and pseudoposterior intervals are not population-calibrated. This does not establish attraction/inhibition of cell locations, patient reproducibility, molecular association, biological compartments, causality, clinical utility, transportability, or a generic multitype framework.

## DEC-0317 — Promote the conditional multitype fit with independent code and a fixed prior grid

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-PP/BACK-01/MRK-02A/WS-40/WS-43/WS-44
- Decision: add one independent NumPyro 0.21.0/JAX 0.11.1 implementation over DEC-0316's exact validated request. Reconstruct the symmetric potential matrix independently and compare every type intercept, unordered potential, gauge-invariant affinity, expected class count, composite conditional score, and one-step expected same-type edge count with the existing Monte Carlo-aware rules. Add a separate five-fit PyMC grid varying only intercept or interaction prior SD by 0.5x/2x around baseline, retaining every full typed result and reporting maximum affinity/composite-score shifts standardized by the baseline pseudoposterior SD under a caller-declared aggregate work and materiality ceiling.
- Consequences: the real fixed-location four-class fit is cross-backend consistent for all 25 compared quantities and stable below its prespecified 0.75-SD threshold over the bounded prior grid. A deliberately separated synthetic control is materially interaction-prior-sensitive, so tests do not encode universal stability. Agreement and this grid do not calibrate composite-posterior uncertainty, normalize the joint Gibbs likelihood, preserve exact class counts under the intercept-only null, add patient replication, or establish attraction, molecular, biological, causal, clinical, significance, or transportability claims. Exact finite-state simulation calibration remains required.

## DEC-0318 — Calibrate the conditional pseudoposterior against an exact finite Gibbs oracle

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-PP/MRK-02A/WS-40/WS-43/WS-44
- Decision: add `conditional-multitype-mark-sbc` for 3–4 types on at most ten fixed graph sites. Enumerate every joint label state under the normalized symmetric categorical Gibbs model, draw intercepts/free potentials from the exact fitted priors, sample labels from the exact finite partition function, and refit the same NumPyro single-site conditional pseudoposterior. Calibrate every free intercept/potential plus every gauge-invariant affinity over 20–100 replicates with exact failure dispositions, rank histograms, 90% coverage, sampler diagnostics, and hard state/enumeration-work/enumeration-byte/iteration/output/time ceilings. Persist the zero-parameter analytic log normalizer `n*ln(K)` as an enumeration oracle.
- Consequences: the implemented three-type small-graph pseudoposterior passes the declared 20-replicate rank and coverage bounds under an exact normalized joint Gibbs prior-generative design. This is calibration evidence for bounded small graphs, not the 2,000-cell real graph, and it does not convert the composite pseudolikelihood into a normalized likelihood, add patient replication, or establish attraction, molecular, biological, causal, clinical, significance, or transportability claims. Representative patient-pattern fitting is the next immediate scientific requirement.

## DEC-0319 — Fit repeated hard-mark patterns with patient as the population unit

- Date: 2026-08-29
- Status: accepted for BAY-03/BAY-PP/BACK-01/FND-04/MRK-02A/WF-01/WS-13/WS-23/WS-41/WS-43
- Decision: add direct and durable `fit-replicated-conditional-multitype-mark` for one balanced immediate CPTAC caller. Reuse DEC-0312's four provenance-sorted patients per MSI/MSS group and two slides per patient, filter to the common Neoplastic/Inflammatory/Connective hard-class vocabulary, and retain at most 512 cells per slide by a stable source-CellId-only SHA-256 rank. Condition on separate exact 50-micrometre slide graphs. Fit separate composition-intercept and symmetric pair-potential hierarchies with baseline and MSI-minus-MSS effects, noncentered patient deviations, and noncentered slide-within-patient deviations; impose reference-type intercept and reference/reference potential zero at every level and report gauge-invariant group and patient affinity contrasts. Bind source/backend/environment/worker/request/hierarchy/radius/graph/sampling identity, hard patient/pattern/point/type/pair/edge/draw/retained-byte/process bounds, and exact-f64 durable replay through the existing scheduler, ledger, artifact store, and transaction.
- Consequences: the immediate hard-multitype caller now estimates patient-population variation without treating cells, edges, or slides as independent biological replicates. The exact 5-by-3/15-by-3 grid oracle recovers the known patient-group interaction shift and exact within-pattern edge counts. The real eight-patient/16-slide fit is diagnostically complete only at the single declared higher-acceptance capacity and all three MSI-minus-MSS affinity intervals span zero. The conditional composite likelihood remains unnormalized, observed locations remain fixed, and one-step conditional checks are not joint posterior simulations. This does not establish equivalence, absence of spatial biology, location-process attraction, causality, clinical utility, external validation, transportability, or a generalized multitype framework; backend agreement, prior sensitivity, and patient-hierarchy calibration remain promotion work.

## DEC-0320 — Compare the patient conditional-mark hierarchy without repeating PyMC

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-PP/BACK-01/WS-40/WS-41/WS-43/WS-44
- Decision: add `replicated-conditional-multitype-mark-agreement` with one independent NumPyro 0.21.0/JAX 0.11.1 implementation. Require a typed PyMC baseline result whose source/backend/request digest exactly matches the freshly prepared hierarchy request, then run only NumPyro over that request rather than starting PyMC again. Independently construct all composition, pair-potential, patient, and pattern terms; compare baseline and group affinities, every patient affinity, all four hierarchy scales, composite score, and every pattern's one-step same-edge and class-count expectations under the existing Monte Carlo-aware rules. Retain exact PyMC/NumPyro/lock/worker/source-request/depth/sampling identities and the existing process/output ceilings.
- Consequences: the real eight-patient/16-slide conditional hierarchy is independently backend-consistent across all 101 compared quantities without spending a second PyMC execution. Both backends are diagnostically complete and all MSI-minus-MSS affinity intervals remain null-compatible. This does not establish prior or hierarchy robustness, calibrate patient-population uncertainty, normalize the pseudolikelihood, add external cohorts, or support equivalence, attraction, biological, causal, clinical, significance, or transportability claims.

## DEC-0321 — Stress the two new hierarchy scales with a fixed bounded grid

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-PP/WS-41/WS-43/WS-44
- Decision: add `replicated-conditional-multitype-mark-sensitivity` for four one-at-a-time 0.5x/2x changes to the patient and pattern-within-patient scale priors around the exact typed PyMC baseline. Reuse the DEC-0320 independent NumPyro request adapter and unchanged data, graph, likelihood, group/intercept/interaction priors, seed, draws, diagnostic thresholds, and depth. Run one to four fixed process shards under an aggregate draw-parameter-work ceiling, retain every full scenario fit in deterministic order, and report group-affinity, patient-affinity, and hierarchy-scale mean shifts standardized only by the baseline PyMC pseudoposterior SD with a caller-declared materiality threshold.
- Consequences: the real four-scenario grid completes in one bounded four-process wave and remains below 0.75 SD; the largest group-affinity and hierarchy-scale shifts are 0.0159 and 0.440 SD. This bounded grid does not prove robustness to likelihood, vocabulary, radius, cell sample, patient selection, group-effect prior, or arbitrary prior families, and it does not calibrate patient-population uncertainty or authorize equivalence, attraction, biological, causal, clinical, significance, or transportability claims.

## DEC-0322 — Calibrate the patient hierarchy against exact finite pattern states

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-PP/WS-40/WS-41/WS-43/WS-44
- Decision: add `replicated-conditional-multitype-mark-sbc` for two groups, four or more independent patients, two or more patterns per patient, 3–4 types, and equal six-to-eight-site pattern graphs. Draw every baseline/group composition and pair-potential term, all four patient/pattern scale parameters, and every noncentered patient/pattern deviation from the exact fitted priors. Conditional on each hierarchy truth, enumerate all `K^N` joint label states separately for every pattern, sample from the exact normalized Gibbs distribution, and refit the unchanged production NumPyro single-site conditional pseudoposterior. Calibrate baseline and group invariant affinities plus all hierarchy scales over 20–100 replicates, retaining exact failure dispositions and enforcing state, enumeration-work, enumeration-byte, iteration, draw, output, and time ceilings. Permit one sampler-capacity retry only when priors, seed, model, estimands, diagnostic thresholds, and acceptance target remain unchanged.
- Consequences: the bounded three-type, four-patient/eight-pattern oracle completes 20/20 after the single capacity increase from 2×1,000 to 2×1,500 retained draws; the first run's five failures were exclusively R-hat 1.01025–1.01312 with adequate ESS/E-BFMI and zero divergences/depth hits. The final calibration covers ten population/hierarchy quantities and retains the analytic per-pattern `N*ln(K)` zero-parameter normalizer. This validates only small exact pattern graphs under the declared prior-generative design; it does not normalize the fitted composite likelihood, prove 8,192-cell calibration, establish equivalence or attraction, or support biological, causal, clinical, significance, external-validation, or transportability claims.

## DEC-0323 — Fit repeated hard types as joint exact-window Cox intensities

- Date: 2026-08-29
- Status: accepted for BAY-03/BAY-PP/BACK-01/FND-04/MRK-02A/WF-01/WS-13/WS-23/WS-41/WS-43
- Decision: add direct and durable `fit-replicated-arbitrary-window-multitype-lgcp` for the same balanced eight-patient/16-slide CPTAC caller as DEC-0312 and DEC-0319. Filter each exact slide event set to the declared complete Neoplastic/Inflammatory/Connective hard-type vocabulary, hash exact source CellId plus type identity, and retain one zero-capable count for every type on every positive 4-by-4 clipped exact-window node. Fit independent type-specific Poisson intensities with type-specific intercept, MSI-minus-MSS, fixed x-covariate, noncentered patient, zero-sum slide-within-patient, and independent fixed-Matérn zero-sum slide-field terms. Report absolute type effects, all pairwise MSI-minus-MSS intensity-effect differences, every hierarchy/node posterior, and per-slide/type total-count and node-variance posterior-predictive checks. Bind exact input/backend/environment/worker/request/window/event/type/kernel/sampling identities and cap patient, slide, type, node, node-type, event, draw-work, result, and process resources through the existing scheduler, ledger, artifact store, exact-f64 codec, and transaction.
- Consequences: the admitted 65,570-cell three-type event partition now has one joint location/type Cox-process likelihood at the patient population unit rather than only a location-conditioned mark pseudolikelihood. The real fit is diagnostically complete and all three type-specific plus all three pairwise MSI-minus-MSS intervals span zero. Independent type-specific latent Cox fields can represent heterogeneous intensity but are not a multitype Gibbs interaction, cross-type latent covariance, inferred kernel, or normalized conditional-mark model. This does not establish equivalence, absence of spatial biology, attraction/inhibition, causality, clinical utility, significance, external validation, or transportability; agreement, sensitivity, calibration, and richer cross-type structure remain promotion work.

## DEC-0324 — Compare every replicated multitype intensity quantity independently

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-PP/BACK-01/WS-40/WS-41/WS-43/WS-44
- Decision: add `replicated-arbitrary-window-multitype-lgcp-agreement` with one independent NumPyro 0.21.0/JAX 0.11.1 implementation. Require an exact typed PyMC result whose input/backend/request digest matches a freshly prepared DEC-0323 request, then start only NumPyro. Independently construct type-specific intercept/group/covariate terms, noncentered patient and within-patient-centered slide effects, independent physical block-Matérn fields with within-slide centering, and the complete node-type Poisson likelihood. Compare all type parameters, pairwise group-effect differences, patient/type and slide/type effects, every node/type latent effect, and every node/type expected count under the existing Monte Carlo-aware interval-overlap and standardized-difference rules with an explicit sampler-specific depth capacity.
- Consequences: the real eight-patient/16-slide/three-type result is independently backend-consistent across all 1,422 quantities without rerunning PyMC. Both backends are diagnostically complete and the null-compatible MSI-minus-MSS findings remain unchanged. Agreement does not establish prior/kernel/hierarchy robustness, calibration, spatial predictive adequacy, pairwise Gibbs interaction, cross-type latent covariance, attraction/inhibition, equivalence, biological significance, causality, clinical utility, external validation, or transportability.

## DEC-0325 — Retain fixed hierarchy and field sensitivity for every replicated type

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-PP/WS-41/WS-43/WS-44
- Decision: add `replicated-arbitrary-window-multitype-lgcp-sensitivity` for eight deterministic one-at-a-time 0.5x/2x changes to patient SD, slide SD, fixed field amplitude, and fixed physical field length around an exact typed PyMC baseline. Reuse the DEC-0324 NumPyro implementation and unchanged data, likelihood, type vocabulary, seed, draws, diagnostics, and tree-depth capacity. Execute one to four bounded process waves under one exact aggregate draw-node-type-work ceiling, retain every full scenario result, and report type-group, pairwise group-difference, patient, slide, hierarchy-scale, and local expected-count shifts standardized only by the baseline PyMC posterior SD.
- Consequences: all eight real 8-patient/16-slide scenarios are diagnostically complete. Type-group and pairwise group-difference shifts stay below 0.127 SD, but field amplitude/length changes move slide effects and local expected counts by as much as 3.42 and 8.56 SD. The null-compatible MSI-minus-MSS contrast is stable under this declared grid, while local reconstruction is materially fixed-kernel-sensitive and is not promoted as invariant. The synthetic oracle retains one divergence for doubled length after its single sampling-capacity increase rather than hiding it. This does not test arbitrary priors, inferred or cross-type kernels, likelihood families, patient selection, biological significance, causality, clinical utility, external validation, or transportability.

## DEC-0326 — Calibrate the exact multitype prior generator against analytic moments

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-PP/WS-40/WS-41/WS-43/WS-44
- Decision: add `replicated-arbitrary-window-multitype-lgcp-prior-calibration` over the exact DEC-0323 typed request through NumPy 2.4.6 in the existing pinned Python lock. Independently generate declared Gaussian and half-normal hierarchy priors, patient and within-patient-centered slide effects, exact Cholesky Matérn fields centered inside each slide, and conditional Poisson counts. Compare five pooled prior families with their analytic moments, every slide's empirical centered-field covariance with the exact centered `HLLᵀH` covariance, and all node-type standardized Poisson residuals with their conditional zero-mean/unit-second-moment oracle. Require 1,024–65,536 deterministic draws and exact simulation-work, conservative Python/NumPy working-byte, process-time, request, environment, and worker identities.
- Consequences: the 4,096-draw real-geometry calibration passes all five prior-moment checks, centered covariance at 0.00752 field-amplitude-squared RMSE, and 2,727,936 Poisson residuals with mean -0.00119 and second moment 0.99993. This verifies the prior generator and likelihood simulation, complementing independent posterior agreement; it is not posterior SBC, posterior predictive adequacy, kernel invariance, cross-type covariance, normalized mark interaction, or biological, causal, clinical, significance, external-validation, or transportability evidence.

## DEC-0327 — Infer one shared physical kernel inside the replicated multitype model

- Date: 2026-08-29
- Status: accepted for BAY-03/BAY-04/BAY-PP/BACK-01/WF-01/WS-13/WS-41/WS-43/WS-44
- Decision: address DEC-0325's material fixed-kernel local sensitivity with direct and durable `fit-replicated-arbitrary-window-multitype-lgcp-inferred-kernel` over the unchanged DEC-0323 patient/type/window request. Infer one positive shared isotropic Matérn-3/2 amplitude and micrometre length scale inside pinned PyMC while retaining independent type-specific fields, type-specific group/covariate/patient/slide terms, exact within-patient and within-slide centering, every node/type posterior, and every slide/type count and node-variance predictive check. Reuse DEC-0314's prespecified HalfNormal amplitude 0.5 and length 1,024-micrometre scales and the existing durable scheduler, ledger, store, transaction, and exact-f64 codec. Bind the exact source/backend/worker/request/kernel-prior/depth identity and cap type/node/draw/process/output plus sum-of-slide-cubes kernel work.
- Consequences: the real 8-patient/16-slide/three-type fit is diagnostically complete and replays byte-identically from a fresh backend-disabled process with one ledger execution. Shared amplitude is 1.035 [0.957, 1.120] and length is 286 [236, 338] micrometres; every absolute and pairwise MSI-minus-MSS interval still spans zero. One shared stationary kernel is smaller than type-specific or correlated cross-type kernels and resolves post-hoc fixed-scale selection only for this immediate caller. It does not establish type/patient/slide-specific scales, cross-type latent covariance, pairwise interaction, local-field calibration, equivalence, absence of biology, significance, causality, clinical utility, external validation, or transportability.

## DEC-0328 — Compare the inferred multitype kernel without rerunning PyMC

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-04/BAY-PP/BACK-01/WS-40/WS-41/WS-43/WS-44
- Decision: add `replicated-arbitrary-window-multitype-lgcp-inferred-kernel-agreement` with an exact typed DEC-0327 PyMC baseline and one independent NumPyro 0.21.0/JAX 0.11.1 implementation. Start only NumPyro after freshly preparing and matching the complete PyMC request. Independently construct both inferred shared kernel parameters, all type-specific fixed and hierarchical effects, dynamic per-slide Matérn Cholesky factors, independent type fields, and every node/type Poisson likelihood term. Compare all type, kernel, pairwise group, patient/type, slide/type, latent-node/type, and expected-count quantities using the prespecified Monte Carlo-aware tolerance and interval-overlap rules with exact backend/lock/worker/source-request/depth identities.
- Consequences: the real inferred-kernel model is independently backend-consistent across all 1,424 quantities without a second PyMC execution. NumPyro is diagnostically complete and estimates amplitude 1.032 and length 284.7 micrometres, agreeing with PyMC's 1.035 and 286.1. This does not provide prior-generative or posterior calibration, type-specific/correlated kernels, pairwise interaction, equivalence, biological significance, causality, clinical utility, external validation, or transportability.

## DEC-0329 — Calibrate the inferred multitype generator on exact real geometry

- Date: 2026-08-29
- Status: accepted for BAY-02/BAY-03/BAY-04/BAY-PP/WS-40/WS-41/WS-43/WS-44
- Decision: add `replicated-arbitrary-window-multitype-lgcp-inferred-kernel-prior-calibration` over the exact DEC-0327 request through NumPy 2.4.6 in the pinned Python lock. Independently draw all five type/hierarchy prior families plus the inferred shared amplitude and micrometre length from their declared Gaussian/half-normal priors. For every draw and slide, construct the dynamic Matérn Cholesky, recover its whitened standard-normal field, impose the production within-slide centering, and generate conditional Poisson counts. Compare seven prior families, whitened first/second moments, centering, and all node/type Poisson residuals with analytic oracles under exact node-type, total kernel-cube, conservative working-byte, output, and time ceilings.
- Consequences: the 4,096-draw exact real-geometry generator passes all seven prior-moment, dynamic-field, and 2,727,936 Poisson-residual checks. This validates prior generation and likelihood simulation for the inferred model, complementing independent posterior agreement. It is not posterior SBC or coverage evidence and does not establish local-field adequacy, type-specific/correlated kernels, pairwise interaction, equivalence, significance, causality, clinical utility, external validation, or transportability.

## DEC-0330 — Measure sparse heat sensitivity to bounded coordinate perturbation

- Date: 2026-08-29
- Status: accepted for FR-01B/GSP-01/WF-01/WS-12/WS-61/WS-62
- Decision: add direct and durable `sparse-radius-heat-stability` for the immediate admitted CellViT sparse-heat caller. Hold exact node IDs and scalar signals fixed, generate 1–64 deterministic SHA-256-seeded independent per-axis coordinate-jitter replicates, rerun the existing exact-radius sparse Chebyshev heat workflow, and report graph changes plus filtered-signal relative L2 change against the unperturbed specimen graph. Admit all runs only when the declared per-run candidate/matrix-vector ceilings fit caller-supplied aggregate worst-case ceilings. Reject non-finite controls/results and zero-baseline nonzero differences. Reuse the existing graph owner, scheduler, ledger, artifact store, exact-f64 codec, and native runtime identity.
- Consequences: one 2,000-cell CPTAC CRC graph now has prespecified 1-micrometre coordinate-perturbation evidence and fresh-process durable replay without a second graph execution. The maximum relative filtered-signal change is 0.00987 against the fixed 0.10 diagnostic threshold. This is one-specimen sensitivity for one signal/radius/time/jitter configuration, not patient reproducibility, segmentation or subsampling calibration, sparse wavelet/scattering scale, molecular association, external validation, biological significance, causality, or clinical utility.

## DEC-0331 — Scale one diffusion-wavelet filter bank without eigendecomposition

- Date: 2026-08-29
- Status: accepted for FR-01B/GSP-01/WF-01/WS-12/WS-62
- Decision: add direct and durable `sparse-radius-diffusion-wavelet` for 1–16 strictly increasing positive diffusion times on the immediate sparse CellViT radius-graph caller. Define a concrete telescoping heat filter bank with first detail `I-H(t0)`, successive details `H(t[j-1])-H(t[j])`, and final coarse `H(t[last])`; compute every heat action through the existing adaptive sparse Chebyshev owner. Retain only detail signals and the final coarse signal, from which every intermediate heat signal and the original input reconstruct. Bind exact graph/request/native identities and enforce per-scale plus aggregate candidate/matrix-vector, peak graph-working, conservative retained-output, and shared durable artifact ceilings.
- Consequences: the four-scale real 2,000-cell graph reconstructs to 3.33e-16 maximum absolute error and replays byte-identically from one ledger execution. The independent small dense spectral oracle agrees at every scale. A rejected redundant schema encoded 1,409,419 bytes over the 1-MiB project ceiling; the retained nonredundant schema encodes 710,433 bytes without increasing shared infrastructure limits. This is a sparse heat filter bank, not a sparse eigensolver, diffusion-wavelet basis tree, scattering transform, patient reproducibility result, molecular association, external validation, biological significance, causality, or clinical utility.

## DEC-0332 — Scale fixed order-two scattering through the sparse heat owner

- Date: 2026-08-29
- Status: accepted for FR-01B/GSP-01/WF-01/WS-12/WS-62
- Decision: add direct and durable `sparse-radius-scattering` for one immediate pathology graph caller using the DEC-0331 telescoping heat filters. Apply pointwise absolute value after every first-order detail; for each strictly coarser scale, apply its heat-difference filter to the first-order modulus and take absolute value again. Aggregate every path only by graph-node mean absolute magnitude and energy, retaining zero-order and final coarse summaries. Support fixed order one or two and 1–16 increasing times. Pre-admit the exact heat-application count and worst-case candidate/matrix-vector work; also cap peak graph working and conservative retained storage. Reuse the existing graph, sparse heat, wavelet, scheduler, ledger, artifact store, and exact-f64 owners.
- Consequences: the real four-scale 2,000-cell caller produces four first-order and six second-order coefficients in exactly 13 sparse heat applications, with independent dense spectral agreement for every small-graph coefficient and identity-final durable replay. This is a fixed descriptive graph scattering summary, not a learned/injective representation, patient-population statistic, radius/segmentation-invariant result, molecular association, external validation, biological significance, causality, or clinical utility.

## DEC-0333 — Retain unstable witness topology under physical coordinate jitter
