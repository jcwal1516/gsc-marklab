...
- Patch imagery/metadata exist, including physical scale and overlap, but no canonical one-vector-per-`PatchId` embedding table or `CellPatchLink` artifact was found.
- Exact sampled-patch selection metadata exist, but the Otsu mask was not persisted in the TCGA/CPTAC inference trees. The honest observation domain is the union of sampled physical patch windows, not the full WSI.
