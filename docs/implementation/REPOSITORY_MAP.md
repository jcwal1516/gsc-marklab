...
| `crates/marklab-embeddings/src/multiscale/records/artifact_graph/**` | Direct-patch eighteen-role validation plus a separate nine-role vector-independent cell-link input graph; role-preserving bindings, exact dependencies, constant-memory managed canonical comparison, redacted failures, and full footprint decode where the cell graph requires it | Runtime prerequisites only; they prove no source-vector/component or opaque-coordinate/anchor correspondence, and derived/support/matrix/patch-region receipts remain separate |
| `crates/marklab-embeddings/src/multiscale/{physical.rs,artifact.rs}` | Central exact C-05 physical schema/manifest/record profiles and private constructors for graph-bound footprint, overlap, assignment, and edge receipts plus a format-neutral paired link receipt | Four physical families are active; support/matrix/patch-region/derived receipts remain pending |
| `crates/marklab-embeddings/src/columnar/multiscale{,/**}` | Shared C-05 physical errors, write summary, caller-budget enforcement, exact cell-link metadata/formulas, and allocation-free footprint/overlap/assignment/edge domain checks | Exact-profile infrastructure only; no general Arrow/Parquet interchange claim |
| `crates/marklab-embeddings/src/columnar/{arrow,parquet}/multiscale/**` | Deterministic footprint/overlap/assignment/edge writers, exact raw-before-stock preflight/readers, publications, managed verification, and receipt entry points | Four of eight C-05 physical families; no matrix or patch-region codec yet |
