...
| `crates/marklab-embeddings/src/multiscale/records/artifact_graph/**` | Direct-patch eighteen-role validation plus separate nine-role vector-independent cell-link and six-role patch-region input graphs; role-preserving bindings, exact dependencies, constant-memory managed canonical comparison, redacted failures, and full footprint decode where each link graph requires it | Runtime prerequisites only; they prove no source-vector/component, opaque-coordinate/anchor correspondence, or geometric patch-region overlap, and derived/support/matrix receipts remain separate |
| `crates/marklab-embeddings/src/multiscale/{physical.rs,artifact.rs}` | Central exact C-05 physical schema/manifest/record profiles, including the three typed matrix manifests, and private constructors for graph-bound footprint, overlap, assignment, edge, and patch-region receipts plus a format-neutral paired cell-link receipt | All eight physical families are active; support/matrix/derived receipts remain pending |
...
| `crates/marklab-embeddings/src/columnar/{arrow,parquet}/multiscale/**` | Deterministic writers, exact raw-before-stock preflight/readers, full borrowed/managed validation, and publication for all eight C-05 families; receipt entry points remain limited to the five graph-authorized support/link families | Exact synthetic physical proof; no broad interchange, matrix receipt, or source promotion |
...
| `tests/multiscale_embedding_artifact_graph/**` | Direct-patch structural fixture plus exact role/profile/dependency/payload/store/privacy/resource tests and separate borrowed/managed footprint/overlap receipt tests in both formats | Sixteen focused tests; structural proof alone intentionally cannot mint a physical receipt, and no support/table/link receipt or promotion exists |
...
| `tests/{multiscale_matrix_columnar.rs,multiscale_matrix_columnar_adversarial.rs,multiscale_matrix_columnar_resources.rs,support/multiscale_matrix_columnar.rs}` | Three typed matrix families in both formats; exact schemas/metadata/records; six fixed byte identities; in-process/fragmenting/fresh-process determinism; raw hostile declarations; borrowed/managed/integrity parity; independent exact/one-short resource oracles; empty, 8,192/8,193/16,386-row, and one-row 65,536-dimension full decode | Sixteen focused tests plus one private Arrow feature guard; no matrix graph/receipt, source correspondence, benchmark, or promotion |
