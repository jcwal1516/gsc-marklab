...
| `crates/marklab-embeddings/src/multiscale/{physical.rs,artifact.rs,matrix_artifact.rs}` | Central exact C-05 physical profiles and private graph-bound receipt constructors; format-neutral cell-link/direct-patch support receipts; compact direct-patch and derived-region table receipts | All eight physical families plus direct-patch and derived-region receipt authority are active; both slide receipt paths remain pending |
...
| `crates/marklab-embeddings/src/columnar/{arrow,parquet}/multiscale/**` | Deterministic writers, exact raw-before-stock preflight/readers, full borrowed/managed validation, and publication for all eight C-05 families; patch and region matrix readers expose candidate/graph-bound borrowed and managed receipt entry points | Exact synthetic physical proof; no broad interchange, slide receipt, source-component correspondence, or source promotion |
...
| `tests/multiscale_embedding_artifact_graph/**` | Direct-patch and derived-region fixtures; exact role/profile/dependency/payload/store/privacy/resource tests; lower support receipts; deterministic region finalization; borrowed/managed Arrow/Parquet patch and region table receipts | Fifty-two focused tests, including seven region-finalizer and three region-receipt cases; no source-component proof, slide path, benchmark, or promotion |
