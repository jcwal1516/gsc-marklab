...
| `tests/multiscale_embedding_records/**` | Strict canonical record/digest/content goldens, four support and two producer modes, dependency sets, negative semantics, empty chains, privacy, and exact resource boundaries | Eleven focused tests plus private row/structure/raw-string/allocation limits cover the source and small-record checkpoints only |
