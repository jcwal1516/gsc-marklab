...
| `crates/marklab-embeddings/src/multiscale/records/artifact_graph/**` | Direct-patch eighteen-role record profiles, role-preserving bindings, exact dependency/DAG checks, constant-memory managed canonical comparison, redacted failures, and runtime-only structural graph token | No physical decode, physical/support/table receipt, source-vector correspondence proof, derived/cell/link graph, adapter admission, or promotion |
...
| `tests/multiscale_embedding_artifact_graph/**` | Direct-patch structural graph fixture plus exact role/profile, seven dependency-shape, eight canonical-payload, supplied-store precedence, privacy, Arrow/Parquet manifest-field, and 4,096-row streaming regressions | Eleven focused tests prove only the direct structural graph; arbitrary digest-valid physical bytes intentionally remain undecoded and unpromoted |
