...
| `crates/marklab-embeddings/src/multiscale/records/artifact_graph/**` | Direct-patch eighteen-role record profiles, role-preserving bindings, exact dependency/DAG checks, constant-memory managed canonical comparison, redacted failures, and runtime-only structural graph token with crate-private footprint/overlap bindings | Structural prerequisite only; it cannot mint a receipt without the separate physical reader and proves no support/table/source correspondence or derived/cell/link graph |
