...
| `crates/marklab-embeddings/src/multiscale/{physical.rs,artifact.rs}` | Central exact C-05 physical schema/manifest/record profiles and private constructors for graph-bound footprint, overlap, assignment, edge, and patch-region receipts plus a format-neutral paired cell-link receipt | Five physical families are active; support/matrix/derived receipts remain pending |
| `crates/marklab-embeddings/src/columnar/multiscale{,/**}` | Shared C-05 physical errors, write summary, caller-budget enforcement, exact cell-link/patch-region metadata and formulas, and allocation-free footprint/overlap/assignment/edge/patch-region domain checks | Exact-profile infrastructure only; no general Arrow/Parquet interchange claim |
| `crates/marklab-embeddings/src/columnar/{arrow,parquet}/multiscale/**` | Deterministic footprint/overlap/assignment/edge/patch-region writers, exact raw-before-stock preflight/readers, publications, managed verification, and receipt entry points | Five of eight C-05 physical families; no matrix codec yet |
