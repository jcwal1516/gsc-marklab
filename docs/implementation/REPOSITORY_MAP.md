...
| `crates/marklab-embeddings/src/multiscale/records/artifact_graph/**` | Direct-patch eighteen-role validation plus a separate nine-role vector-independent cell-link input graph; role-preserving bindings, exact dependencies, constant-memory managed canonical comparison, redacted failures, and full footprint decode where the cell graph requires it | Runtime prerequisites only; they prove no source-vector/component or opaque-coordinate/anchor correspondence, and assignment/edge/derived/link receipts remain separate |
| `crates/marklab-embeddings/src/multiscale/{physical.rs,artifact.rs}` | Central exact C-05 physical schema/manifest/record profiles and private constructors for graph-bound footprint and overlap runtime receipts | Only footprint and overlap receipts exist; support/matrix/assignment/edge/patch-region/derived receipts remain pending |
| `crates/marklab-embeddings/src/columnar/multiscale.rs` | Shared C-05 physical errors, write summary, caller-budget enforcement, and allocation-free footprint/overlap domain checks | Exact-profile infrastructure only; no general Arrow/Parquet interchange claim |
| `crates/marklab-embeddings/src/columnar/{arrow,parquet}/multiscale/**` | Deterministic footprint/overlap writers, exact raw-before-stock preflight/readers, publications, managed verification, and receipt entry points | First two of eight C-05 physical families; no matrix, assignment, edge, or patch-region codec yet |
...
| `tests/cell_patch_input_artifact_graph.rs` | Dedicated vector-independent nine-role graph, canonical payload/profile/dependency drift, managed availability/integrity precedence, and full footprint decode | Four focused tests; no assignment/edge bytes, pair receipt, source-coordinate adapter correspondence, vector binding, or promotion |
