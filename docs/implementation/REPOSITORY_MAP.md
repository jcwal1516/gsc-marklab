...
| `crates/marklab-embeddings/src/multiscale/records/artifact_graph/**` | Direct-patch eighteen-role validation plus separate nine-role vector-independent cell-link and six-role patch-region input graphs; role-preserving bindings, exact dependencies, constant-memory managed canonical comparison, redacted failures, and full footprint decode where each link graph requires it; the direct token retains the verified provenance logical identity needed by its table receipt | Runtime prerequisites only; they prove no source-vector/component, opaque-coordinate/anchor correspondence, or geometric patch-region overlap, and derived graphs remain separate |
| `crates/marklab-embeddings/src/multiscale/{physical.rs,artifact.rs,matrix_artifact.rs}` | Central exact C-05 physical schema/manifest/record profiles; private constructors for graph-bound footprint, overlap, assignment, edge, and patch-region receipts; format-neutral paired cell-link and direct patch-support receipts; compact direct-patch table receipt | All eight physical families and direct-patch receipt authority are active; region/slide support/table receipts and derived finalization remain pending |
...
| `crates/marklab-embeddings/src/columnar/{arrow,parquet}/multiscale/**` | Deterministic writers, exact raw-before-stock preflight/readers, full borrowed/managed validation, and publication for all eight C-05 families; patch matrix readers additionally expose graph/support/source-row-bound borrowed and managed direct receipt entry points | Exact synthetic physical proof; no broad interchange, derived matrix receipt, source-component correspondence, or source promotion |
...
| `tests/{multiscale_embedding_arrow.rs,multiscale_embedding_parquet.rs,support/multiscale_columnar.rs}` | Shared synthetic spatial fixture plus exact profile/golden, deterministic, hostile declaration, raw/stock, borrowed/managed, multi-group, and independent budget-oracle coverage | Footprint/overlap only; C-05 fuzz, differential all-family corpus, DHAT, and scale closure remain pending |
| `tests/multiscale_embedding_artifact_graph/**` | Direct-patch structural fixture plus exact role/profile/dependency/payload/store/privacy/resource tests; separate footprint/overlap receipts; format-neutral patch-support composition; borrowed/managed Arrow/Parquet patch-table receipt verification; source-row/binding/integrity/boundary regressions | Twenty-six focused tests, including ten direct support/table receipt cases; no source-component adapter proof, derived receipt, or promotion |
...
| `tests/{multiscale_matrix_columnar.rs,multiscale_matrix_columnar_adversarial.rs,multiscale_matrix_columnar_resources.rs,support/multiscale_matrix_columnar.rs}` | Three typed matrix families in both formats; exact schemas/metadata/records; six fixed byte identities; in-process/fragmenting/fresh-process determinism; raw hostile declarations; borrowed/managed/integrity parity; independent exact/one-short resource oracles; empty, 8,192/8,193/16,386-row, and one-row 65,536-dimension full decode | Sixteen focused physical tests plus one private Arrow feature guard; direct-patch receipt authority is tested with the graph suite, while derived receipts, source correspondence, benchmark, and promotion remain pending |
