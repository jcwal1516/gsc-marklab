...
- Provenance DAG: version one admits direct patch extraction plus deterministic region/slide derivation only. Strict canonical source chain, normalization, four support variants, weighted/arithmetic derivation, cell-link producer, and exhaustive assessment values/codecs now exist. The four multiscale provenance variants, exact artifact-record profiles, verified receipts, and graph validation remain unimplemented. Opaque source-vector or coordinate availability never proves component or anchor correspondence without a later admitted adapter receipt.
