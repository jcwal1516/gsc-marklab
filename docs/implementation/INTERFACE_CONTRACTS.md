...
- Provenance DAG: version one admits direct patch extraction plus deterministic region/slide derivation only. Strict canonical source-entity, source-to-expected identity-map, source-row-link, and normalization values/codecs now exist; model/execution records, support, derivation, producer/assessment records, verified receipts, and graph validation remain unimplemented. Opaque source-vector or coordinate availability never proves component or anchor correspondence without a later admitted adapter receipt.
