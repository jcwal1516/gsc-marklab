...
- Provenance DAG: version one admits direct patch extraction plus deterministic region/slide derivation only. Strict canonical source chain, normalization, four support variants, weighted/arithmetic derivation, cell-link producer, exhaustive assessment, and all four multiscale provenance values/codecs now exist. Exact artifact-record profiles, verified receipts, and graph validation remain unimplemented. Opaque source-vector or coordinate availability never proves component or anchor correspondence without a later admitted adapter receipt.
