...
- Vector-free links: cell assignments store each anchor once and reference shared patch rows through complete all-containing or explicit one-to-four interpolation edges, including closed zero-edge states. Patch-region links encode one exhaustive producer declaration with exact reduced overlap fractions. Hierarchy proves ancestry, not geometry; FND-02 retains geometric truth.
- Provenance DAG: version one admits direct patch extraction plus deterministic region/slide derivation only. Exact source entities, identity map, row link, normalization, model/execution records, support, and lower-level verified receipts form an acyclic publication graph. Opaque source-vector or coordinate availability never proves component or anchor correspondence without a later admitted adapter receipt.
