...
## RUST-MIGRATION-01 — Native grouped-conformal checkpoint, 2026-09-05

Baseline `f784302`; primary checkout. The native grouped-conformal milestone is complete;
all other production Python families remain in scope. Exact measurements, raw samples, hashes,
negative reference findings and commands are in `audits/grouped_conformal_measurements.json`.

| Gate | Executed command/evidence | Result and limit |
|---|---|---|
| Numerical red/green | `cargo +1.96.0 test --locked -p marklab-numerics --test bfgs` | Missing API red; final 4/4 pass, including planted positive quadratics, nonfinite/resource errors and exhausted convergence. |
| Native domain and internals | `env CARGO_TARGET_DIR=target/native-migration/focused cargo +1.96.0 test --locked -p marklab-bayes --test native_grouped_conformal`; same package `--lib grouped_conformal` | Final 9 domain + 3 internal tests pass. Independent small/300-patient references, analytic null, near-collinearity/minimum alpha, leakage, legacy reading, ordering, gradient and semantic-hash checks. |
| Final arithmetic regression | same package `--lib prediction_standardizes_before_multiplication` | Expected red: finite standardized products were rejected after raw-product overflow. Green after standardizing before multiplication, matching Python; the helper is confined to grouped-conformal prediction and its legacy validator. |
| Application/process boundaries | `cargo +1.96.0 test --locked -p marklab --test bayes_grouped_conformal_cli --test cli_discovery --test workflow_contract --test workspace_contract`; `cargo +1.96.0 test --locked -p marklab --lib native_backend` | Initial 6 CLI, 5 discovery, 7 workflow, 2 ownership and 3 lifecycle cases pass. Final all-feature CLI rerun in the focused artifact directory passes 6/6 after the arithmetic correction. |
| Native CI selection | `env CARGO_TARGET_DIR=target/native-migration/focused MARKLAB_DISABLE_EXTERNAL_BACKEND_EXECUTION=1 MARKLAB_PYTHON=/nonexistent/marklab-python MARKLAB_RUNTIME_ROOT=/nonexistent/marklab-runtime cargo +1.96.0 nextest run --locked --workspace --all-features --test bayes_grouped_conformal_cli --test native_grouped_conformal --test bfgs` | 19/19 pass, zero skipped. This is the existing Rust job's new native selection; no Python environment is installed by that job. The corresponding CI contract failed red and passed green. |
| Formatting/lint | `cargo +1.96.0 fmt --all --check`; `cargo +1.96.0 clippy --locked --workspace --all-targets --all-features -- -D warnings`; CLI-only variant `--no-default-features --features cli` | Pass. All-feature workspace Clippy was rerun after the contained arithmetic correction (1m34s). CLI-only lint preceded that correction; no cfg or API changed in it. |
| Minimal/physical features | `cargo +1.96.0 check --locked --workspace --no-default-features`; `cargo +1.96.0 check --locked --workspace --all-targets --no-default-features --features csv` | Pass. CSV-only emits 14 existing test-instrumentation warnings allowed by WORKSPACE_POLICY. The full phase/release feature matrix was not run. |
| Documentation | `env RUSTDOCFLAGS=-Dwarnings cargo +1.96.0 doc --locked --no-deps --workspace --all-features`; `cargo +1.96.0 test --locked --workspace --doc --all-features` | Pass: strict docs and all seventeen doc-test targets (zero cases). These preceded the contained arithmetic correction and final ledger updates. |
| Default-concurrency workspace | `cargo +1.96.0 nextest run --locked --workspace --all-features` | Exit 100: 457 passed, one unchanged Gaussian crossed/nested hierarchy hit its 180-second Python deadline; 1266 selected tests were not run, 28 existing skips. No assertion or timeout was weakened. |
| Timeout investigation | `cargo +1.96.0 test --locked --all-features --test bayes_gaussian_crossed_nested_hierarchy_cli` | The unchanged test passes in isolation, 117.78 s. Concurrent load is the supported explanation, not a claimed production fix. |
| Full CI-concurrency rerun | `cargo +1.96.0 nextest run --locked --workspace --all-features --test-threads 2` | 1724/1724 pass in 2849.705 s, 12 slow and 28 existing skips. This used the compiled snapshot before the final contained arithmetic correction and CI-contract addition. Current affected tests and Clippy pass afterward; a fresh unfiltered 1725-test run is not claimed. |
| Corrected release comparison | `cargo +1.96.0 build --locked --release --bin marklab`; benchmark harness `--native-warm` and `--baseline ... --candidate ...` commands in the audit | Corrected build 8m12s. Ten alternating paired repetitions per successful workload, exact sets/identities/counts and method-specific float tolerances. Cold median paired speedups 8.09x/8.26x/7.29x; warm 7.59x/5.19x/4.45x. All reported intervals exceed one. Runs followed completion of task builds and workspace tests. Local synthetic evidence only. |
| Failed-reference investigation | `env PYTHONDONTWRITEBYTECODE=1 target/pymc-venv/bin/python tests/python/benchmark_grouped_conformal.py --audit-failures target/native-migration/focused/release/examples/grouped_conformal_benchmark` | All eleven native fits have finite frozen-Python objectives and maximum absolute gradients below 1e-8; fixed-parameter downstream calculations agree. Python optimizer failures remain failures and provide no independent fit/speedup comparison. |
| Criterion/profiling | `env MARKLAB_BENCH_PROFILE=smoke cargo +1.96.0 bench --locked --package marklab --bench grouped_conformal -- --quick`; samply command in audit | Both Criterion workloads pass. The 13m17s bench build includes the full-debug CLI. Its smoke overlapped the workspace suite and preceded the final arithmetic correction; use corrected release comparisons for timing claims. The earlier 10304-sample native profile motivated removal of allocated matrix JSON hashing. |
| Package ownership | `env CARGO_TARGET_DIR=target/native-migration/focused cargo +1.96.0 package --locked -p marklab-numerics -p marklab-bayes --allow-dirty --offline` | Both scoped packages verify after correcting a sibling-source include. An earlier same-version temporary-registry retry saw stale contents; the fresh artifact directory resolved that. Not full-workspace registry/publication admission; final contained arithmetic does not change the package boundary. |
| CI lint availability | `actionlint .github/workflows/ci.yml` | Exit 127, command not found. PyYAML is also absent from the locked scientific environment. Ruby Psych safe loading verifies YAML syntax and the native environment; CI contract and exact native selection pass locally. These are not equivalent to actionlint or hosted CI. |
| Scope/identity preservation | Frozen Git-blob inventory generation; master/lock digest and tracker checks; final diff/status review | 154 unique source files, original caller links retained, all 233 tracker IDs and master-plan/Python-worker/Python-lock/Cargo-lock bytes preserved. Ownership proposals and 24 unmatched literal production callers require individual admission. |

No global dependency, fuzz, RSS-scale, release-matrix, hosted CI or cross-platform gate was claimed.
