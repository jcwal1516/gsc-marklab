...
def main():
    ...
    for path in sorted(paths):
        ...
        writer.writerow([revision,path,hashlib.sha256(sources[path].encode()).hexdigest(),";".join(sorted(imports)),";".join(production),";".join(tests),";".join(contracts),owner(stem),"native: bounded parity and synthetic performance gates passed" if stem in ("marklab_scipy_grouped_conformal_worker", "marklab_scipy_prediction_calibration_worker") else "pending", "EMB-CONFORMAL-01 / RUST-MIGRATION-01" if "grouped_conformal" in stem else "EMB-CALIBRATE-01 / RUST-MIGRATION-01 (two legacy cold failures retained)" if stem=="marklab_scipy_prediction_calibration_worker" else "existing caller fixtures; not yet individually admitted"])
