...
Last updated: 2026-08-24T02:29:50-04:00
...
- Committed baseline before the current checkpoint: `d27e70f75dc2c5d7fd2aaeb56c3a9b11f1141994`
...
- Current phase/workstream/task: Phase 2 / WS-C / C-06 declared scalar-pattern workflow complete; broader FND-04/C-06 remains active
...
- Planned next: the next dependency-ordered observable scientific computation or user workflow with an immediate production caller; no general mark/format/geometry infrastructure is pre-authorized
...
- C-06 now has an immediately called measurement-origin computation plus one observable declared scalar-pattern engine/project workflow. The compatibility `Pattern` remains unchanged, but the versioned Rust boundary supplies canonical CellId/frame/status/provenance/threshold identity without copying arrays. FND-04/C-06 and EMB-01 remain active because general mark kinds, arbitrary units/modalities, missingness, physical/file adapters, observation windows, spatial weights, new nulls/inference, durable result provenance, and real-source promotion remain absent.
...
- C-06 declared scalar workflow only: one borrowed versioned input, compact runtime identity/routing summaries, direct engine delegation, one separate local workflow node, exact provenance/profile checks, and focused integration evidence. No physical format, receipt, validator framework, dependency, lockfile, source adapter, corpus mutation/promotion, CLI/config/result/Pattern/loader, general MarkTable, geometry/window/weights abstraction, new estimator, or biological claim changed.
...
1. Audit the dependency-ordered master-plan frontier for the next observable scientific computation or user workflow; FND-02 remains a no-go until exact frame/source/topology/window prerequisites and an immediate caller coexist.
...
```bash
cargo +1.96.0 test --locked --test scalar_mark_input --test declared_marked_workflow
```
