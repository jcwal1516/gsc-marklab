...
| BACK-01 | active | Six static typed PyMC 6.3.0 workflows and POT 0.9.7.post1 run durably with exact Python-3.12 lock/worker/license/schema/control identity; pinned NumPyro 0.21.0/JAX 0.11.1 supplies Gaussian/Student-t/beta-binomial hierarchy, beta-binomial group-regression, and fixed-grid-LGCP cross-backend callers. Capability/security manifests, doctor/admission validation, CmdStan, GPU evidence, and broader backend classes remain. |
| BAY-01 | active | Typed Gaussian, Student-t, beta-binomial hierarchy, beta-binomial group-regression, and exact fixed-grid LGCP models fit independently in PyMC and NumPyro with agreement within declared Monte Carlo/absolute rules on synthetic and real CellViT inputs; unified broader likelihood/predictor/field/point-process IR coverage and CmdStan remain. |
| BAY-02 | active | PyMC and NumPyro NUTS diagnostics, audited SMC, differentiated/nested Laplace, PSIS-LOO/comparison, conjugate/Gaussian/Student-t/beta-binomial/group-regression/fixed-grid-LGCP SBC, and corresponding sensitivity grids are runnable; CmdStan, actual GPU evidence, and broader model-family calibration remain. |
| BAY-03 | active | Gaussian and finite-variance Student-t hierarchies run on 284 ROIs nested in 121 patients; a beta-binomial hierarchy runs on 178 patients/1,542,389 exact CellViT annotation trials; and a patient-unit MSI/MSS beta-binomial regression runs on 105 labeled patients/886,751 trials. All pass durable replay where applicable, PyMC/NumPyro agreement, PPC, bounded sensitivity, and SBC. Multiple-covariate, repeated, crossed, varying-slope, ordinal/hurdle, and spatial hierarchies remain. |
...
| BAY-HIER-A | active | Gaussian known-sigma, robust Student-t, beta-binomial patient, and patient-unit beta-binomial MSI/MSS group models have real execution, PyMC/NumPyro agreement, PPC, sensitivity, and SBC; applicable fits replay durably, and exact-conjugate resource-distance response plus site meta-regression also run. Multiple covariates, ordinal/hurdle, crossed/nested levels, and random slopes remain. |
...
| PLAT-01 | active | Immutable artifacts, catalog/store/cache, exact source/runtime identities, durable heads/ledger/recovery/replay, and typed unified dependency-free native/PyMC/POT execution exist, including six PyMC workflows; dependency-bearing multi-node orchestration remains. |
...
| WS-12 | active | Typed durable execution covers native/POT nodes, six PyMC fits including a two-input LGCP, real beta-binomial count hierarchy, and real beta-binomial molecular-group regression, plus marked-result/Moran three-node graphs with cross-process replay; schema-bound inputs use the existing verified store path. General DAG construction, resource planning, workflow schema, parallel execution, and whole-graph resume remain. |
...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; Gaussian/Student-t/beta-binomial hierarchies, patient beta-binomial group regression, and fixed-grid LGCP pass two-backend agreement, sensitivity, and SBC, while CmdStan, GPU evidence, broader families, and generalized artifacts remain. |
| WS-41 | active | Durable Gaussian and robust Student-t partial pooling run on real ROI-within-patient data; durable beta-binomial pooling runs on 178 real CellViT patients; and durable patient-unit MSI/MSS beta-binomial regression runs on the exact 105-patient label/count intersection. All pass PyMC/NumPyro agreement, PPC, sensitivity, and SBC. Repeated/crossed models, multiple covariates, and random slopes remain. |
