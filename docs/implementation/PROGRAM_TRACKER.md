...
| BACK-01 | active | Three static typed PyMC 6.3.0 workflows and POT 0.9.7.post1 run durably with exact Python-3.12 lock/worker/license/schema/control identity; pinned NumPyro 0.21.0/JAX 0.11.1 now supplies closed hierarchy and fixed-grid-LGCP cross-backend callers. Capability/security manifests, doctor/admission validation, CmdStan, GPU evidence, and broader backend classes remain. |
| BAY-01 | active | The typed Gaussian patient hierarchy and exact fixed-grid LGCP now fit independently in PyMC and NumPyro with agreement within Monte Carlo error on synthetic and real CellViT inputs; unified broader likelihood/predictor/field/point-process IR coverage and CmdStan remain. |
| BAY-02 | active | PyMC and NumPyro NUTS diagnostics, audited SMC, differentiated/nested Laplace, PSIS-LOO/comparison, conjugate/hierarchy/fixed-grid-LGCP SBC, and hierarchy/LGCP sensitivity are runnable; CmdStan, actual GPU evidence, and broader model-family calibration remain. |
...
| WS-44 | active | The Gaussian patient hierarchy and exact fixed-grid LGCP both pass PyMC/NumPyro agreement, posterior predictive checks, one-at-a-time sensitivity, and SBC alongside existing PSIS-LOO/comparison and conjugate calibration; broader likelihood/hierarchy/field families, CmdStan, and actual GPU evidence remain. |
