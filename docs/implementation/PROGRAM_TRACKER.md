...
| BACK-01 | active | Three static typed PyMC 6.3.0 workflows and POT 0.9.7.post1 run durably with exact Python-3.12 lock/worker/license/schema/control identity; pinned NumPyro 0.21.0/JAX 0.11.1 now supplies one closed cross-backend hierarchy caller. Capability/security manifests, doctor/admission validation, CmdStan, GPU evidence, and broader backend classes remain. |
| BAY-01 | active | The typed Gaussian patient hierarchy now fits in PyMC and NumPyro with agreement within Monte Carlo error on synthetic and real CellViT ROI data, satisfying the first two-backend promotion model; unified broader likelihood/predictor/field/point-process IR coverage and CmdStan remain. |
| BAY-02 | active | PyMC and NumPyro NUTS diagnostics, audited SMC, differentiated/nested Laplace, PSIS-LOO/comparison, conjugate and Gaussian-hierarchy SBC, plus conjugate/hierarchical prior sensitivity are runnable; field/point-process SBC and sensitivity remain. |
| BAY-03 | active | Durable Gaussian patient varying intercepts run on 284 real ROIs nested in 121 patients, agree across PyMC/NumPyro, pass PPC, a five-fit 0.5x/2x prior grid, and 40-replicate SBC; repeated, crossed, non-Gaussian, varying-slope, and spatial hierarchies remain. |
...
| BAY-HIER-A | active | The Gaussian known-sigma patient hierarchy has durable real execution, PyMC/NumPyro agreement, PPC, prior sensitivity, and SBC; exact-conjugate resource-distance response and site meta-regression also run. Unknown dispersion, Student-t, binomial/count/ordinal/hurdle, crossed/nested levels, and random slopes remain. |
...
| BAY-PP | active | Exact rectangular IPP/grid/SPDE LGCP, cluster/Strauss/Geyer, exact finite exchange/multitype, latent-parent/replicated cluster, marked/embedding constructors, replicated LGCP, and PPC are runnable; one durable PyMC LGCP now fits all 500 cells in an exact real CellViT patch, while cross-backend/SBC calibration, broader spatial PPC, fitted marks, and arbitrary windows remain. |
| BAY-PP-A | active | Exact rectangle-grid LGCP plus IC-0200 rectangular SPDE LGCP are runnable with conserved counts; one real 4x4 CellViT patch fit has normalized diagnostics and count PPC, while cross-backend agreement, spatial-summary checks, adaptive refinement, calibration, arbitrary windows, and inferred SPDE hyperparameters remain. |
...
| EMB-01 | active | Exact synthetic methods now have real row-bound CPTAC evidence for raw 1,280-dimensional and split-safe projected vector variograms plus patient-held-out analysis; broader geometry/edge correction, technical calibration, external agreement, and stable promotion remain. |
...
| FND-02 | active | Exact bounded ObservationWindow2D now owns canonical topology, closed membership, indexed unsigned/signed distance, and optional exact installed physical-[X,Y]-micrometre frame identity consumed by global Moran; polygon compartments, external differential evidence, and broader promotion remain. |
...
| FND-04 | active | A row-aligned typed MarkTable now binds stable CellIds plus binary, probability, finite continuous, and provenance-bearing histologic-compartment categorical columns; declared marked analysis and global Moran are immediate callers. Ordinal, simplex, vector-reference, general categorical/interchange, real-IHC, and scale coverage remain. |
...
| FND-06 | active | Public InferenceDesign now owns exact blocked whole-unit schedules for patient-label and cell-mark random labeling, including level/null/unit/seed namespace/alternative/single-endpoint multiplicity; existing CSR, paired, functional, and Max-T designs remain method-owned. Repeated/multisite/cluster/interference integration and broader multiplicity ownership remain. |
...
| SIG-01F | active | A bounded typed scalar semivariogram now has exact provenance/frame/bin/pair-plan identity plus deterministic global or compartment-blocked whole-value ERL simultaneous inference and hard work ceilings; edge-correction decisions, directionality, external agreement, scale, patient comparison, and real validation remain. |
...
| WS-12 | active | Typed durable execution covers native/POT nodes, three PyMC fits including a two-input LGCP, and marked-result/Moran three-node graphs with cross-process replay; schema-bound inputs use the existing verified store path. General DAG construction, resource planning, workflow schema, parallel execution, and whole-graph resume remain. |
| WS-13 | active | Durable PyMC Normal-mean, patient-hierarchy, gridded-LGCP and POT FGW workflows share one closed static descriptor with exact environment/adapter/license/schema/control identity; one direct NumPyro agreement caller is pinned. Generalized discovery, broader backend classes, doctor/admission validation, and capability/security manifests remain. |
...
| WS-22 | blocked with named prerequisite | Exact bounded 2-D windows, canonical unsigned/signed boundary distance, optional physical-frame binding, and K/L plus Moran geometry callers are complete; polygon compartments, volumes, external geometry agreement, and broader shared plans remain. |
| WS-23 | active | Declared analysis and global Moran share a bounded row-aligned MarkTable with binary/probability/continuous plus one typed histologic-compartment categorical specialization; remaining general categorical/ordinal/simplex/vector/interchange/real-data families stay open. |
| WS-24 | active | Real CPTAC CellViT source/model/environment/protocol identity, raw/projected row correspondence, coordinates, annotations, patient identity, and cell-patch links are verified and bundled; canonical importer integration and independent patch-vector sources remain. |
...
| WS-32 | active | Real raw-1,280 and split-safe projected CellViT variograms now supplement the synthetic vector/covariance and typed scalar workflows; scalar edge correction, external agreement, broader scale validation, and stable promotion remain. |
...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; the Gaussian hierarchy passes two-backend agreement, sensitivity, and SBC, while CmdStan, GPU evidence, field/point-process calibration, and generalized artifacts remain. |
| WS-41 | active | Durable patient Gaussian partial pooling runs on real ROI-within-patient data and passes PyMC/NumPyro agreement, PPC, prior sensitivity, and SBC; one-covariate site meta-regression also runs, while broader likelihoods, repeated/crossed models, and multiple covariates remain. |
...
| WS-43 | active | Fixed/fitted rectangular IPP, Berman–Turner refinement, exact-cell fixed-hyperparameter LGCP construction/fitting/pattern replicas, and cluster/Gibbs workflows are runnable; the durable real CellViT patch fit adds count PPC, while cross-backend/SBC calibration, spatial PPC envelopes, arbitrary windows, and broader multitype/marked/replicated fitting remain. |
| WS-44 | active | The Gaussian patient hierarchy passes PyMC/NumPyro agreement, PPC, prior sensitivity, and SBC alongside existing PSIS-LOO/comparison and conjugate calibration; field/point-process cross-backend agreement, SBC, sensitivity, and spatial PPC remain the next Phase 4 blockers. |
| WS-50 | active | Distance/kernel/covariance/envelope methods now have promotable real CellViT provenance, physical scale, raw/projected variograms, and patient-held-out evidence; broader graph-scale calibration, external agreement, and stable release policy remain. |
