...
| GEO-01D | active | Exact vector-polygon component areas drive largest-component fraction, area entropy, and perimeter/area; separately declared physical-radius cell graphs drive binary and complete multiclass mixing matrices, cross-edge fractions, per-class neighbor entropy, and random-label expectations. Multiscale stability, uncertainty, patient inference, and real stable-ID validation remain. |
...
| WF-01 | active | Typed keys, store-verified semantic artifacts, exact admission, composition, and replay include K/L, F/G/J, compartment interface/contact/fragmentation, binary/multiclass/soft cell mixing, categorical/probability connection, continuous correlation/weighted K, cross-K, real TCGA/Schürch fingerprints, and marked/Moran graphs. General schema, heterogeneous construction, resource planning, parallel scheduling, and whole-graph resume remain. |
...
| WS-23 | active | Declared scalar methods plus hard multiclass mixing and soft probability-simplex composition/fixed-radius neighborhoods share the bounded row-aligned MarkTable. CSV/Parquet categorical codebooks are retained and declaration-checked; broader interchange/export and canonical stable-ID real-data import remain. |
...
| WS-25 | active | CSV/Parquet Pattern ingestion now preserves ordered categorical codebooks and codes through one typed boundary. SpatialData, AnnData, OME-NGFF, direct MarkTable Arrow/Parquet/Zarr, stable cell-ID import, and export remain. |
| WS-30 | active | Standard-border point/mark methods, binary/multiclass compartment-mark mixing, and fixed/multiscale soft-simplex neighborhoods share exact window/index/geometry owners, deterministic traversal, hard limits, and durable execution. Translation/isotropic corrections, multiple intensity estimators, and broader calibration remain. |
