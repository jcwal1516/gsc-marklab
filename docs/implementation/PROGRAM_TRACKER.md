...
| GEO-01D | active | Exact vector-polygon component areas drive largest-component fraction, area entropy, and perimeter/area; separately declared physical-radius cell graphs drive binary and complete multiclass mixing matrices, cross-edge fractions, per-class neighbor entropy, and random-label expectations. Real stable-ID validation passes; multiscale stability, full-simplex uncertainty, and patient inference remain. |
...
| WF-01 | active | Typed keys, store-verified semantic artifacts, exact admission, composition, and replay include K/L, F/G/J, compartment interface/contact/fragmentation, binary/multiclass/soft cell mixing, real stable-ID CellViT mixing, categorical/probability connection, continuous correlation/weighted K, cross-K, real TCGA/Schürch fingerprints, and marked/Moran graphs. General schema, heterogeneous construction, resource planning, parallel scheduling, and whole-graph resume remain. |
...
| WS-12 | active | Typed durable execution covers native/POT, PyMC, spatial point/mark methods, compartment geometry/mixing, real stable-ID CellViT multiclass mixing, soft simplex composition/neighborhoods, marked/Moran graphs, and real TCGA/Schürch workflows with cross-process replay. General DAG/resource/parallel/whole-graph resume remains. |
...
| WS-23 | active | Declared scalar methods plus hard multiclass mixing and soft probability-simplex composition/fixed-radius neighborhoods share the bounded row-aligned MarkTable. CSV/Parquet codebooks and canonical source CellIds are retained and declaration-checked; broader interchange/export remains. |
