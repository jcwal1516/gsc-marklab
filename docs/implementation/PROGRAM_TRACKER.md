...
| FND-06 | active | Public InferenceDesign owns exact whole-unit schedules for patient-label and cell-mark random labeling. Real TCGA and Schürch fingerprint energy/MMD tests now use patient labels and patient as the population unit alongside CSR, paired, functional, and Max-T designs. Repeated/multisite/cluster/interference integration and broader multiplicity ownership remain. |
...
| PLAT-01 | active | Immutable artifacts, exact identities, durable heads/ledger/recovery/replay, and typed native/PyMC/POT execution now drive real TCGA M0–M4/M6 and Schürch CODEX patient workflows plus seven PyMC workflows. The canonical CRC bundle hashes all selected evidence and replay proofs; dependency-bearing multi-node orchestration remains. |
...
| WF-01 | active | Typed keys, exact artifact admission, sequential composition, and durable replay now include bounded fixed-shard real TCGA M0–M4/M6 and Schürch CODEX patient workflows, plus the marked and Moran three-node graphs. General workflow schema, heterogeneous construction, resource planning, parallel scheduling, and whole-graph resume remain. |
...
| WS-12 | active | Typed durable execution covers native/POT nodes, pinned PyMC fits, marked/Moran graphs, and real patient-held-out TCGA M0–M4/M6 plus Schürch CODEX retrieval with byte-identical cross-process replay. Fixed project shards bound the current work; general DAG construction, resource planning, parallel scheduling, and whole-graph resume remain. |
