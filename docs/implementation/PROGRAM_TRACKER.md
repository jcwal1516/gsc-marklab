...
| TOP-01 | active | All Part VIII functions have bounded experimental workflows through IC-0158–IC-0164, and pinned witness persistence now has admitted 2,000-cell execution plus a truthfully unstable 16-replicate coordinate-jitter diagnostic; a prespecified patient endpoint and replicated validation remain missing. |
| TOP-01A | active | Pinned exact alpha/witness persistence, transforms, patient comparison, and validation controls are runnable; one stable-ID physical-coordinate CellViT witness filtration has direct/durable coordinate-perturbation evidence, while patient replication and subsampling/segmentation stability remain missing. |
...
| WS-63 | active | Bounded topology and morphology are live, including one 2,000-cell admitted CellViT witness complex with measured resource use, durable replay, and an unstable prespecified 1-micrometre coordinate-jitter diagnostic; pathology-linked patient endpoints, subsampling/segmentation stability, and replicated cohorts remain missing. |
