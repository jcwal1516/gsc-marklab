...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; Gaussian/Student-t/beta-binomial hierarchies, patient unadjusted/gender-adjusted beta-binomial and multiclass Dirichlet-multinomial group regressions, and fixed-grid LGCP pass two-backend agreement, sensitivity, and SBC. CmdStan, GPU evidence, broader families, and generalized artifacts remain. |
| WS-41 | active | Durable Gaussian/Student-t ROI-within-patient, 178-patient beta-binomial, 105-patient binary and five-class MSI/MSS regressions, and a 217-slide-within-105-patient repeated-slide hierarchy run on real CellViT inputs. The multiclass caller passes diagnostics, replay, independent backend agreement, sensitivity, and synthetic calibration; cohort and distinct ROI-within-slide effects, crossed models, additional covariates, and random slopes remain. |
