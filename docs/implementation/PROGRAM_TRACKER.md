...
| BAY-03 | active | Gaussian/Student-t ROI hierarchies, beta-binomial/multiclass patient models, patient-level proportional-odds, exposure-aware hurdle, and nine-site ordinal varying-slope likelihoods plus a 217-slide repeated-count hierarchy run on real data. The TCGA ordinal site hierarchy converges but retains category-profile lack of fit; the CPTAC hurdle fit is null/uncertain. Cohort, distinct ROI-within-slide, crossed, non-proportional ordinal, calibrated richer fields, and additional identified structures remain. |
