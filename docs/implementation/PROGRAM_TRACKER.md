...
| FND-04 | active | A row-aligned typed MarkTable binds stable CellIds plus binary, scalar probability, continuous, histologic categorical, probability-simplex, and one verified CellViT vector-artifact-reference specialization with exact status/provenance. Ordinal, broader categorical/vector/interchange, and real-IHC coverage remain. |
| FND-05 | active | A verified 366-slide CPTAC CellViT corpus supplies exact provenance and 1,542,389 finite raw rows; one centroid caller now consumes its existing verified table/artifact through a no-copy row-bound typed vector reference. Canonical source-import promotion, remaining typed vector callers, and independent patch vectors remain. |
| FND-06 | active | Public InferenceDesign owns exact whole-unit schedules for patient-label, patient population-independence MMD/energy, and complete cell-mark random labeling. Conditional whole-pattern/fixed-pilot nulls and complete-row pair nulls remain method-bound; real TCGA/Schürch tests use patients. Broader paired/blocked fingerprint, repeated/multisite/cluster/interference, and multiplicity ownership remain. |
...
| MRK-02D | active | Three formal vector-statistic callers are runnable; the binary-centroid caller now consumes a canonical row-bound no-copy CellViT artifact reference with exact status/provenance and store-aware replay. The probability/covariance callers and stable real source-import promotion remain. |
...
| WS-24 | active | Real TCGA/CPTAC CellViT provenance, raw correspondence, coordinates, annotations, hierarchy, cell-patch metadata, and counts are verified and bundled; one existing verified cell-vector artifact now has a no-copy typed MarkTable reference. Canonical source-import promotion remains, and independent genuine patch-vector tensors remain absent rather than substituted by cell vectors. |
...
| WS-31 | active | Conditional CSR, reused ERL, scalar/paired/functional patient permutation, patient population-independence MMD/energy, single-step Max-T, complete-vector embedding nulls, and shared blocked patient-label/cell-mark/variogram schedules are runnable; remaining null families, calibration, and broader multiplicity require further FND-06 slices. |
