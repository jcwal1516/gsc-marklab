...
| PP-03 | active | Standard-border and exact polygon translation-corrected homogeneous Epanechnikov g, directed two-level cross-g, and Gaussian/exact-compartment inhomogeneous g specializations use explicit pair bandwidth, typed empty support, appropriate whole-pattern or complete-row nulls, ERL, hard bounds, and durable replay. The translation family reuses one exact overlap per contributing displacement without changing border bytes. Isotropic/multitype corrections, pinned spatstat agreement, and broader calibration remain. |
| PP-03A | active | Bounded standard-border and translation-corrected homogeneous, Gaussian-pilot inhomogeneous, and exact binary-compartment inhomogeneous compact-support g specializations have independent arithmetic/GEOS/direct-loop oracles, appropriate conditioned-pattern inference, finite/resource guards, and user-facing durable replay. Each corrected/inhomogeneous result retains its exact edge or intensity artifact while keeping pair bandwidth explicit. The real Gaussian pilot is inadequate and unpromoted; a real binary partition is unavailable. Isotropic correction, pinned external agreement, and broader null/edge calibration remain. |
...
| WS-30 | active | Standard-border point/mark methods, translation/isotropic homogeneous K/L, translation-corrected homogeneous g, Gaussian and exact binary-compartment inhomogeneous K/L/g, prespecified Gaussian bandwidth-selected K/L, binary/multiclass mixing, real four-radius categorical connection/cross-K/cross-g, and fixed/multiscale soft-simplex neighborhoods share exact window/index/geometry owners, deterministic traversal, hard limits, and durable execution. Isotropic g, pinned external agreement, and broader calibration remain. |
