...
| BACK-01 | active | Four static typed PyMC 6.3.0 workflows and POT 0.9.7.post1 run durably with exact Python-3.12 lock/worker/license/schema/control identity; pinned NumPyro 0.21.0/JAX 0.11.1 supplies Gaussian/Student-t hierarchy and fixed-grid-LGCP cross-backend callers. Capability/security manifests, doctor/admission validation, CmdStan, GPU evidence, and broader backend classes remain. |
| BAY-01 | active | The typed Gaussian and Student-t patient hierarchies plus exact fixed-grid LGCP fit independently in PyMC and NumPyro with agreement within declared Monte Carlo/absolute rules on synthetic and real CellViT inputs; unified broader likelihood/predictor/field/point-process IR coverage and CmdStan remain. |
| BAY-02 | active | PyMC and NumPyro NUTS diagnostics, audited SMC, differentiated/nested Laplace, PSIS-LOO/comparison, conjugate/Gaussian/Student-t/fixed-grid-LGCP SBC, and hierarchy/LGCP sensitivity are runnable; CmdStan, actual GPU evidence, and broader model-family calibration remain. |
| BAY-03 | active | Gaussian and finite-variance Student-t patient varying intercepts run on 284 real ROIs nested in 121 patients; both pass durable replay, PyMC/NumPyro agreement, PPC, bounded sensitivity, and SBC. A typed beta-binomial hierarchy is synthetic-oracle complete; real count admission plus repeated, crossed, varying-slope, and spatial hierarchies remain. |
...
| BAY-HIER-A | active | Gaussian known-sigma and robust Student-t patient hierarchies have durable real execution, PyMC/NumPyro agreement, PPC, sensitivity, and SBC; typed beta-binomial, exact-conjugate resource-distance response, and site meta-regression also run. A real count table plus ordinal/hurdle, crossed/nested levels, and random slopes remain. |
...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; Gaussian/Student-t hierarchies and fixed-grid LGCP pass two-backend agreement, sensitivity, and SBC, while CmdStan, GPU evidence, broader families, and generalized artifacts remain. |
| WS-41 | active | Durable Gaussian and robust Student-t partial pooling run on real ROI-within-patient data and pass PyMC/NumPyro agreement, PPC, prior/tail sensitivity, and SBC; typed beta-binomial and one-covariate site meta-regression also run, while a real count caller, repeated/crossed models, and multiple covariates remain. |
