...
| FND-04 | active | A row-aligned typed MarkTable binds stable CellIds plus binary, scalar probability, continuous, histologic categorical, measured-IHC ordinal, probability-simplex, and verified CellViT vector-reference columns with exact status/provenance. Complete simplex rows now support composition, multiscale neighborhoods, and full source-target expected pair mixing with durable replay. CSV/Parquet retain codebooks/source CellIds; broader interchange/export and real full-simplex CellViT input remain. |
| FND-05 | active with named missing data | A verified 366-slide CPTAC CellViT corpus supplies exact provenance and 1,542,389 finite raw rows; formal vector callers consume its verified no-copy reference. Cell records expose hard type plus only winning-class confidence, not a complete class vector; independent patch vectors also remain absent. Canonical source-import promotion and broader interchange remain. |
...
| PP-02 | active with negative real-scale evidence | Standard-border inhomogeneous K/L and g now consume both the persisted Gaussian leave-one-out/fixed-grid pilot and an exact binary-compartment `(n_c-1)/area_c` estimator. Both use shared inverse-intensity pair/center normalization, explicit conditioned nulls, ERL, hard bounds, and durable replay. The real 512-cell Gaussian caller fails its grid-mass/intensity-range diagnostic, while a real exact binary partition is unavailable. Pinned `Kinhom/Linhom/pcfinhom` agreement, broader calibration/corrections, and a stable real scale policy remain. |
...
| PP-03A | active | Bounded homogeneous, Gaussian-pilot inhomogeneous, and exact binary-compartment inhomogeneous compact-support g specializations have independent direct-loop oracles, appropriate conditioned-pattern inference, finite/resource guards, and user-facing durable replay. Each inhomogeneous result persists exactly the same estimator/null artifact as its K/L caller while keeping pair bandwidth distinct. The real Gaussian pilot is inadequate and unpromoted; a real binary partition is unavailable. Pinned external agreement and broader null/edge calibration remain. |
...
| PP-05 | active | The explicit-bandwidth Gaussian estimator persists `n/(n-1)` leave-one-out event rows, deterministic window-mass quadrature, fixed probe intensities/masses, exact digests, finite floors, and hard bounds consumed by inhomogeneous K/L and g. A second exact binary-compartment estimator persists `(n_c-1)/area_c` rows, rejects interface/sparse roles, fixes both compartment counts under its exact-window null, and runs durably through both K/L and g. Broader cross-fitting, bandwidth selection, real partition evidence, and pinned external agreement remain. |
...
| WS-23 | active | Declared scalar methods plus hard multiclass mixing/pair curves and soft probability-simplex composition, multiscale neighborhoods, and full expected pair matrices share the bounded row-aligned MarkTable. CSV/Parquet codebooks and canonical source CellIds are retained; broader interchange/export and real full-simplex CellViT input remain. |
...
| WS-30 | active | Standard-border point/mark methods, Gaussian and exact binary-compartment inhomogeneous K/L/g, binary/multiclass mixing, real four-radius categorical connection/cross-K/cross-g, and fixed/multiscale soft-simplex neighborhoods share exact window/index/geometry owners, deterministic traversal, hard limits, and durable execution. Translation/isotropic corrections, bandwidth selection, pinned external agreement, and broader calibration remain. |
