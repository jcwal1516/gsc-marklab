...
| FND-02 | active | Exact bounded ObservationWindow2D owns polygon/multipolygon/hole topology, closed membership, indexed unsigned/signed distance, and physical-frame identity. One exact segment-aligned binary compartment tessellation now supplies an oriented shared-interface index and typed per-cell interface profiles with GEOS agreement and durable replay. Multiclass/residual/uncertain partitions, volumes, and broader external geometry agreement remain. |
| FND-03 | active | One exact SpatialGeometryPlan2D/SpatialIndex2D serves homogeneous/inhomogeneous K/L, g, F/G/J, and the shared once-retained directed radius-pair plan now consumed by cross-g, categorical/probability connection, continuous mark correlation, and mark-weighted K; observed/null evaluations reuse exact geometry under hard bounds. General edge-weight/bin plans and broader adoption remain. |
...
| GEO-01 | active | One frame-bound exact binary compartment partition provides oriented shared-interface distance to typed cell rows without aliasing the tissue edge. Contact, multiclass, uncertainty, objects, morphology, patient inference, and real pathology validation remain. |
...
| GEO-01B | active | The exact oriented shared-interface length is live; the next caller must divide it by an explicit compartment-boundary denominator and retain outer-versus-internal boundary roles. Broader contact definitions, uncertainty, and real validation remain. |
...
| WF-01 | active | Typed keys, store-verified semantic artifacts, exact admission, composition, and replay include K/L, F/G/J, compartment-interface profiles, categorical/probability connection, continuous correlation/weighted K, cross-K, real TCGA/Schürch fingerprints, and marked/Moran graphs. General schema, heterogeneous construction, resource planning, parallel scheduling, and whole-graph resume remain. |
...
| WS-12 | active | Typed durable execution covers native/POT, PyMC, homogeneous/inhomogeneous K/L and g, cross-g, F/G/J, exact compartment-interface profiles, store-verified categorical/probability connection, continuous correlation/weighted K, cross-K, marked/Moran graphs, and real TCGA/Schürch workflows with cross-process replay. General DAG/resource/parallel/whole-graph resume remains. |
...
| WS-22 | active | Exact bounded polygon/multipolygon/hole windows, signed/unsigned boundary distance, physical-frame binding, shared spatial callers, and one segment-aligned binary compartment/interface path are live. It has independent GEOS agreement, typed cell profiles, hard bounds, and durable replay. Multiclass/residual/uncertain partitions, contact/morphology operators, volume windows, and broader external agreement remain. |
...
| WS-30 | active | Standard-border homogeneous/inhomogeneous K/L and g, homogeneous cross-g, F/G/J, categorical/probability connection, continuous correlation/weighted K, and directed cross-K share exact window/index/geometry owners, deterministic nulls, ERL, hard limits, and durable execution. Translation/isotropic corrections, multitype inhomogeneous g, multiple intensity estimators, and broader calibration remain. |
