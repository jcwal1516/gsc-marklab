...
| BAY-01 | active | Typed Gaussian, Student-t, beta-binomial hierarchy, unadjusted and gender-adjusted beta-binomial group regressions, exact fixed-grid LGCP, and patient Dirichlet-multinomial multiclass group composition are runnable; the earlier families fit independently in PyMC and NumPyro within declared agreement rules, while the new multiclass likelihood still requires independent backend agreement/sensitivity/SBC before promotion. Unified broader likelihood/predictor/field/point-process IR coverage and CmdStan remain. |
...
| MRK-02A | active | One exact categorical MarkTable specialization supplies a directed source/target type pair, unknown/empty-level failures, a shared pair/null plan, cross-K, cross-g, and real stable-ID CellViT execution. General many-pair multiplicity and broader interchange remain. |
...
| WS-13 | active | Durable PyMC Normal-mean, Gaussian/Student-t/beta-binomial patient hierarchies, beta-binomial and Dirichlet-multinomial group regressions, gridded-LGCP, and POT FGW workflows share one closed static descriptor with exact environment/adapter/license/schema/control identity; direct NumPyro agreement callers are pinned for the promoted earlier families. Generalized discovery, broader backend classes, doctor/admission validation, and capability/security manifests remain. |
...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; Gaussian/Student-t/beta-binomial hierarchies, patient unadjusted and gender-adjusted beta-binomial group regressions, and fixed-grid LGCP pass two-backend agreement, sensitivity, and SBC. Patient Dirichlet-multinomial multiclass composition is now runnable in PyMC with real evidence but still requires that promotion sequence; CmdStan, GPU evidence, broader families, and generalized artifacts remain. |
| WS-41 | active | Durable Gaussian/Student-t ROI-within-patient, 178-patient beta-binomial, 105-patient binary and five-class MSI/MSS regressions, and a 217-slide-within-105-patient repeated-slide hierarchy run on real CellViT inputs. The new multiclass caller passes its declared diagnostics and replay but still needs backend agreement/sensitivity/calibration; cohort and distinct ROI-within-slide effects, crossed models, additional covariates, and random slopes remain. |
