...
| FND-04 | active | A row-aligned typed MarkTable binds stable CellIds plus binary, scalar probability, continuous, histologic categorical, and contiguous probability-simplex columns with exact status/provenance. Soft composition/entropy and fixed-radius soft neighborhoods consume complete CellViT-compatible rows without thresholding. Ordinal, vector-reference, broader categorical/interchange, real-IHC, and multiscale simplex coverage remain. |
...
| NIC-01 | active | Complete probability-simplex rows now support one fixed-radius soft neighborhood composition with exact geometry and durable replay. Multiscale stability, niches/domains, patient inference, and real validation remain. |
| NIC-01A | active | One fixed micrometre radius reports per-cell expected neighbor class mass, directed-incidence aggregate mass, typed zero-neighbor states, exact identities, hard pair/memory bounds, independent oracle, and durable replay. Prespecified multiscale shared-plan output and real stability remain. |
