...
| PP-03B | active | Directed cross-K and two-level Epanechnikov cross-g include standard-border and exact polygon translation correction with exact source/target counts, typed empty support, complete-row random-label envelopes, hard bounds, and durable CLI replay. Standard-border cross-g has eight-patient real evidence; translation cross-g has bounded one-specimen capacity evidence. Isotropic, inhomogeneous intensity, broader multitype coverage, patient replication, and pinned external agreement remain. |
