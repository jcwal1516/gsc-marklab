...
| BAY-03 | active | Gaussian/Student-t ROI hierarchies, beta-binomial/multiclass patient models, a patient-level proportional-odds group likelihood, and a 217-slide repeated-count hierarchy run on real data; durable eight-patient/16-slide scalar LGCP, conditional hard-mark, and fixed/inferred-kernel multitype location-process hierarchies are runnable with null-compatible group results. The 126-patient TCGA stage fit converges but retains material category-profile lack of fit. Cohort, distinct ROI-within-slide, crossed, varying-slope, hurdle, calibrated richer fields, and additional identified structures remain. |
...
| BAY-HIER-A | active | Gaussian known-sigma, robust Student-t, beta-binomial patient, patient-unit unadjusted plus gender-adjusted beta-binomial MSI/MSS group models, and one patient-level ordinal MSI/MSS model have real execution and applicable durable replay; the promoted count families retain PyMC/NumPyro agreement, PPC, sensitivity, and SBC, while exact-conjugate resource-distance response plus site meta-regression also run. Hurdle, crossed/nested levels, random slopes, and promotion/calibration of the ordinal family remain caller-dependent. |
