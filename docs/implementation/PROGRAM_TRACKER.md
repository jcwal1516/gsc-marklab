...
| FND-04 | active | A row-aligned typed MarkTable binds stable CellIds plus binary, scalar probability, continuous, histologic categorical, measured-IHC ordinal, probability-simplex, and verified CellViT vector-reference columns with exact status/provenance. Complete simplex rows support composition, multiscale neighborhoods, and full source-target expected pair mixing; opt-in continuous nucleus-area CSV now reaches durable scalar-variogram inference without changing categorical identities. CSV/Parquet retain codebooks/source CellIds; broader interchange/export and real full-simplex CellViT input remain. |
