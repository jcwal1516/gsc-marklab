...
| FND-06 | active | Public InferenceDesign owns exact whole-unit schedules for patient-label and complete cell-mark random labeling. Conditional whole-pattern CSR drives homogeneous K/L, g, and F/G/J; one fixed gridded intensity drives conditioned inhomogeneous K/L and g nulls; categorical cross-g, probability, and continuous pair nulls permute complete rows without rebuilding geometry; real TCGA/Schürch tests use patients. Broader repeated/multisite/cluster/interference and multiplicity ownership remain. |
