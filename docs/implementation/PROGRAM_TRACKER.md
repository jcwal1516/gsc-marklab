...
| BAY-02 | active | PyMC and NumPyro NUTS diagnostics, audited SMC, differentiated/nested Laplace, PSIS-LOO/comparison, conjugate SBC, and conjugate posterior/predictive/decision prior sensitivity are runnable; hierarchical/field/point-process SBC and sensitivity remain. |
| BAY-03 | active | Durable Gaussian patient varying intercepts now run on 284 real ROIs nested in 121 patients and agree across PyMC/NumPyro with partial pooling and predictive checks; repeated, crossed, non-Gaussian, varying-slope, spatial-hierarchy, SBC, and sensitivity work remains. |
...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; the Gaussian hierarchy passes two-backend agreement, while CmdStan, GPU execution evidence, broader SBC/sensitivity, and generalized artifacts remain. |
| WS-41 | active | Durable patient Gaussian partial pooling runs on real ROI-within-patient data and agrees across PyMC/NumPyro; one-covariate site meta-regression also runs, while broader likelihoods, repeated/crossed models, multiple covariates, SBC, and sensitivity remain. |
...
| WS-44 | active | The Gaussian patient hierarchy now passes PyMC/NumPyro agreement alongside existing PSIS-LOO/comparison, conjugate SBC, and conjugate prior sensitivity; hierarchical SBC/sensitivity plus field/point-process cross-backend calibration remain the next Phase 4 blockers. |
