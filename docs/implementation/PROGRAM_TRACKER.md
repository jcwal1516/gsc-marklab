...
| BAY-03 | active | Gaussian/Student-t ROI hierarchies, beta-binomial/multiclass patient models, patient-level proportional-odds, exposure-aware hurdle, site-varying, valid non-proportional ordinal likelihoods, and one fixed identified Gaussian ROI/slide/patient/cohort/crossed-batch/random-slope hierarchy are runnable. Non-proportional TCGA category PPC and whole-site-held-out score improve overall with mixed site directions. Real crossed/nested/multicohort estimation remains input-blocked; calibrated richer fields and additional identified structures remain. |
| BAY-04 | active | Exact/multi-output/VFE GP, predictive process, NNGP, rectangular and exact-window boundary-adaptive SPDEs, and one bounded prespecified local-anisotropic/nonstationary/interior-adaptive α=2 FEM specialization are runnable. Learned local parameters, arbitrary multi-output nonstationarity, scalable sparse fits, and real promotion remain. |
...
| BAY-HIER-A | active | Gaussian known-sigma, robust Student-t, beta-binomial patient, patient-unit unadjusted plus gender-adjusted beta-binomial MSI/MSS group models, and patient-level ordinal and hurdle MSI/MSS models have real execution and applicable durable replay; the promoted count families retain PyMC/NumPyro agreement, PPC, sensitivity, and SBC, while exact-conjugate resource-distance response plus site meta-regression also run. Crossed/nested levels, random slopes, and promotion/calibration of the ordinal/hurdle families remain caller-dependent. |
...
| BAY-PP | active | Exact rectangular IPP/grid/SPDE LGCP, cluster/Strauss/Geyer, multitype, marked, replicated, and PPC mechanics are runnable; exact-window physical PPC, scalar/multitype fixed/inferred-kernel LGCPs, and type-specific inhomogeneous cross-g with standard/translation/isotropic correction replay durably. Bounded multitype rank SBC exists. Patient-replicated joint location/mark and Gaussian/finite-variance-Student-t location/embedding fits are runnable. A real 6-patient/12-slide correspondence-complete Student-t fit replays durably but is nonconverged with failed embedding PPC and an uncertain held-out spatial increment; it remains diagnostic-only. Pinned external agreement and additional identified coupled models remain caller-dependent. |
...
| DATA-01 | active | Identity, coordinates, artifacts, embeddings and borrowed scalar identity are implemented. ARCH-INTEGRATION-01 adds nullable named quantitative/binary/categorical assay columns, explicit physical slide inputs and a durable multiplex panel-to-patient caller. Broader modality families, transforms and durable physical interchange remain. |
...
| EMB-01 | active | Exact methods now have real row-bound TCGA/CPTAC raw 1,280-dimensional summaries and physical-scale vector variograms, bounded cell/field stability, patient/site-held-out retrieval, orthogonal Schürch CODEX/H&E evidence, and one exact-window/quadrature/projected-cell patient-replicated joint table. Its real robust joint fit is nonconverged and fails embedding PPC, so the molecular-class evidence remains null-to-negative; direct same-schema external agreement and stable promotion remain. |
...
| PLAT-01 | active | Immutable artifacts, exact identities, durable heads/ledger/recovery/replay, typed native/PyMC/POT execution, and one bounded parallel/resumable marked DAG are live. One composed CLI tree now owns discovery and execution routing across existing adapters. A library-owned multiplex application now connects strict recipes, slide summaries, equal-slide patient Max-T, durable resume and atomic claim-bounded reports through the same Rust/CLI/Python service. Other binary-owned applications, general project construction and arbitrary heterogeneous orchestration remain. |
...
| WS-12 | active | Typed durable execution covers native/POT/PyMC/scientific nodes plus a fixed three-node marked DAG with explicit resource waves, parallel roots, roots-only interruption, cross-process comparison resume, and all-hit replay. The multiplex application adds serial bounded slide/collection/patient dependencies, a reproducible partial checkpoint and exact replay without rerunning completed statistics. General heterogeneous DAG construction and arbitrary whole-graph scheduling remain. |
| WS-13 | active | Durable PyMC patient hierarchies, group regressions, gridded/exact-window IPP/LGCP, the replicated exact-window patient hierarchy, and POT FGW share one closed static descriptor with exact environment/adapter/license/schema/control identity; direct NumPyro agreement callers are pinned for promoted families. Runtime asset discovery and the bounded installation doctor now serve existing adapters; the required backend CI lane provisions the lock. Both runners share a cleared-environment startup policy that honors the hash seed; PyMC workers count per-transition divergence flags. Broader backend classes, clean hosted/cross-platform execution, and capability/security manifests remain. |
...
| WS-92 | active | A standard-library Python client and separately locked real AnnData/H5AD test profile invoke the same native multiplex recipe/application, including partial resume and exact replay. No statistics are duplicated. Broader project/artifact clients, R bindings, zero-copy transport, notebooks and arbitrary recipes remain in Phase 9 scope. |
