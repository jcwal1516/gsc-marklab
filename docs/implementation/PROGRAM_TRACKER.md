...
| PP-03B | active | Directed cross-K and two-level Epanechnikov cross-g include standard-border, exact polygon translation, and source-centred visible-arc isotropic correction with exact source/target counts, typed empty support, complete-row random-label envelopes, hard bounds, and durable CLI replay. Standard-border cross-g has eight-patient evidence; corrected families have bounded one-specimen capacity evidence. Inhomogeneous intensity, broader multitype coverage, corrected patient replication, and pinned external agreement remain. |
