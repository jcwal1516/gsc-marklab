...
| BAY-03 | active | Gaussian/Student-t ROI hierarchies, beta-binomial/multiclass patient models, patient-level proportional-odds, exposure-aware hurdle, site-varying, and valid non-proportional ordinal likelihoods plus a 217-slide repeated-count hierarchy run on real data. Non-proportional TCGA category PPC passes in-sample and its whole-site-held-out log score improves overall, with mixed site directions retained. Cohort, distinct ROI-within-slide, crossed, calibrated richer fields, and additional identified structures remain. |
