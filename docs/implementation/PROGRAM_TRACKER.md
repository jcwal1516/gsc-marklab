...
| BACK-01 | active | Five static typed PyMC 6.3.0 workflows and POT 0.9.7.post1 run durably with exact Python-3.12 lock/worker/license/schema/control identity; pinned NumPyro 0.21.0/JAX 0.11.1 supplies Gaussian/Student-t/beta-binomial hierarchy and fixed-grid-LGCP cross-backend callers. Capability/security manifests, doctor/admission validation, CmdStan, GPU evidence, and broader backend classes remain. |
| BAY-01 | active | Typed Gaussian, Student-t, and beta-binomial patient hierarchies plus exact fixed-grid LGCP fit independently in PyMC and NumPyro with agreement within declared Monte Carlo/absolute rules on synthetic and real CellViT inputs; unified broader likelihood/predictor/field/point-process IR coverage and CmdStan remain. |
| BAY-02 | active | PyMC and NumPyro NUTS diagnostics, audited SMC, differentiated/nested Laplace, PSIS-LOO/comparison, conjugate/Gaussian/Student-t/beta-binomial/fixed-grid-LGCP SBC, and hierarchy/LGCP sensitivity are runnable; CmdStan, actual GPU evidence, and broader model-family calibration remain. |
| BAY-03 | active | Gaussian and finite-variance Student-t hierarchies run on 284 ROIs nested in 121 patients, and a beta-binomial hierarchy runs on 178 patients/1,542,389 exact CellViT annotation trials; all three pass durable replay, PyMC/NumPyro agreement, PPC, bounded sensitivity, and SBC. Group effects plus repeated, crossed, varying-slope, ordinal/hurdle, and spatial hierarchies remain. |
...
| BAY-HIER-A | active | Gaussian known-sigma, robust Student-t, and beta-binomial patient hierarchies have durable real execution, PyMC/NumPyro agreement, PPC, sensitivity, and SBC; exact-conjugate resource-distance response and site meta-regression also run. Real group effects plus ordinal/hurdle, crossed/nested levels, and random slopes remain. |
...
| PLAT-01 | active | Immutable artifacts, catalog/store/cache, exact source/runtime identities, durable heads/ledger/recovery/replay, and typed unified dependency-free native/PyMC/POT execution exist, including five PyMC workflows; dependency-bearing multi-node orchestration remains. |
...
| WS-12 | active | Typed durable execution covers native/POT nodes, five PyMC fits including a two-input LGCP and real beta-binomial count hierarchy, and marked-result/Moran three-node graphs with cross-process replay; schema-bound inputs use the existing verified store path. General DAG construction, resource planning, workflow schema, parallel execution, and whole-graph resume remain. |
| WS-13 | active | Durable PyMC Normal-mean, Gaussian/Student-t/beta-binomial patient hierarchies, gridded-LGCP and POT FGW workflows share one closed static descriptor with exact environment/adapter/license/schema/control identity; direct NumPyro agreement callers are pinned. Generalized discovery, broader backend classes, doctor/admission validation, and capability/security manifests remain. |
...
| WS-24 | active | Real CPTAC CellViT source/model/environment/protocol identity, raw/projected row correspondence, coordinates, annotations, patient identity, cell-patch links, and exact per-patient five-class counts are verified and bundled; canonical importer integration and independent patch-vector sources remain. |
...
| WS-40 | active | Typed PyMC and NumPyro NUTS, PyMC SMC, differentiated/nested Laplace, and pinned ArviZ diagnostics/PSIS-LOO are runnable; Gaussian/Student-t/beta-binomial hierarchies and fixed-grid LGCP pass two-backend agreement, sensitivity, and SBC, while CmdStan, GPU evidence, broader families, and generalized artifacts remain. |
| WS-41 | active | Durable Gaussian and robust Student-t partial pooling run on real ROI-within-patient data, while durable beta-binomial pooling runs on real CellViT patient counts; all pass PyMC/NumPyro agreement, PPC, sensitivity, and SBC. Group effects, repeated/crossed models, and multiple covariates remain. |
...
| WS-44 | active | Gaussian, Student-t, and beta-binomial patient hierarchies plus the exact fixed-grid LGCP pass PyMC/NumPyro agreement, posterior predictive checks, one-at-a-time sensitivity, and SBC alongside existing PSIS-LOO/comparison and conjugate calibration; broader likelihood/hierarchy/field families, CmdStan, and actual GPU evidence remain. |
