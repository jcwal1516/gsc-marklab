...
| BAY-03 | active | Gaussian/Student-t ROI hierarchies, beta-binomial/multiclass patient models, patient-level proportional-odds and exposure-aware hurdle group likelihoods, and a 217-slide repeated-count hierarchy run on real data; durable eight-patient/16-slide scalar LGCP, conditional hard-mark, and fixed/inferred-kernel multitype location-process hierarchies are runnable with null-compatible group results. The 126-patient TCGA ordinal fit retains category-profile lack of fit, while the 105-patient CPTAC hurdle fit is null/uncertain. Cohort, distinct ROI-within-slide, crossed, varying-slope, calibrated richer fields, and additional identified structures remain. |
