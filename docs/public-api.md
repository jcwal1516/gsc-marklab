...
| --- | --- | --- | --- |
| `python_backend_assets_root`, `python_backend_interpreter`, `python_backend_cache`, `PythonBackendRuntimeError`, `python_backend_command` | Share installed runtime selection and the curated process-start policy across scientific adapters without coupling library clients to CLI parsing. | Concrete environment-variable and failure contracts; asset admission remains separate from interpreter admission and replay. The expert Command builder requires callers to retain its cleared environment and safe import policy. | [Python backends](python-backends.md); `python_backend_runtime`, `python_worker_startup`, real-fit and topology replay tests. |
| `AssayMarkDeclaration`, `AssayMarkValues`, `ScalarMarkColumn::assay`, `MarkTable::assay_column` | Admit named nullable assay quantities, binary observations and nominal codebooks in the existing row-aligned table. | New assay-bearing tables use logical identity v2; legacy constructors, public modality/unit/error enums and compatibility-only identity v1 remain unchanged. Legacy Pattern projection retains its strict semantics. | [Multiplex workflow](multiplex-study.md); `multiplex_panel`, `scalar_mark_input`. |
| `AssaySpatialInput`, `AssaySpatialLimits`, `AssaySpatialOutcome`, `AssaySpatialSummary`, `AssaySpatialUnavailable`, `summarize_assay_spatial` | Evaluate existing Moran/Geary arithmetic on one admitted observed-cell graph per selected channel. | Physical frame/window, finite values, selected-channel identity and work bounds are explicit; missing rows are excluded without imputation. | `multiplex_panel` independent graph oracle, missingness and resource cases; existing typed Moran workflows. |
| `analyze_multiplex_study`, `execute_multiplex_study`, `publish_multiplex_study`, `MultiplexStudyResult`, `MultiplexStudyRun`, `MultiplexStudyTarget` | One library-owned panel-to-patient application, directly called by the CLI and thin Python client. | Concrete experimental recipe/result v1, separate from result 0.3. Durable slide/collection/cohort nodes reuse existing scheduler and transaction owners. No arbitrary workflow registry or clinical promotion. | [Multiplex workflow](multiplex-study.md); `multiplex_study`, `multiplex_study_cli`, real AnnData client tests. |
...
| `ResultDocument`, `AnalysisResult`, `Provenance`, `RESULT_FORMAT_VERSION` | Read, write, and discriminate versioned analysis documents; safely convert the documented 0.2 marked subset. | Exact JSON shape is the result-format 0.3 contract; unsupported or ambiguous legacy states are rejected. | 0.3 and 0.2→0.3 round trips, conversion-limit, pre/post, unknown-version/field/variant tests. |
