...
| --- | --- | --- | --- |
| `python_backend_assets_root`, `python_backend_interpreter`, `python_backend_cache`, `PythonBackendRuntimeError` | Share installed runtime path resolution across scientific adapters without coupling library clients to CLI parsing. | Concrete environment-variable and failure contracts; asset admission remains separate from interpreter admission and replay. | [Python backends](python-backends.md); `python_backend_runtime` tests and relocated real-fit replay. |
